...
struct system_info {
    ...
    struct {
        ...
        struct {
            ...
            uint32_t txm_addr_space;
            uint32_t txm_trust_level;
            uint32_t sw_asid;
            uint32_t wx_allowed;
            uint32_t type;
        } pmap;
        ...
        struct {
            uint32_t allows_invalid_code;
            uint32_t main_region;
        } txm_address_space;

        struct {
            uint32_t code_signature;
        } txm_region;

        struct {
            uint32_t fork_trust_pair;
        } txm_code_signature;

        struct {
            uint32_t pmap_cs_region_next;
            uint32_t pmap_cs_region_right;
            uint32_t cd_entry;
        } pmap_cs_region;

        struct {
            uint32_t pmap_cs_code_directory_next;
            uint32_t signing_identifier;
            uint32_t team_identifier;
    ...
    /*
	 * Relaxin appends its serialized kernel symbols so the upstream field
	 * offsets above remain unchanged for existing BaseBin consumers.
	 */
    struct {
        uint64_t sptm_args;
    } relaxinKernelSymbol;
...
static inline bool system_info_uses_sptm(void) {
    return gSystemInfo.relaxinKernelSymbol.sptm_args != 0;
}
...
#define KERNEL_STRUCTS_ITERATE(ctx, iterator) \
	...
	\
	iterator(ctx, kernelStruct.label.sandbox); \
	\
	...
	iterator(ctx, kernelStruct.pmap.txm_addr_space); \
	iterator(ctx, kernelStruct.pmap.txm_trust_level); \
	iterator(ctx, kernelStruct.pmap.sw_asid); \
	iterator(ctx, kernelStruct.pmap.wx_allowed); \
	iterator(ctx, kernelStruct.pmap.type); \
	\
	iterator(ctx, kernelStruct.txm_address_space.allows_invalid_code); \
	iterator(ctx, kernelStruct.txm_address_space.main_region); \
	iterator(ctx, kernelStruct.txm_region.code_signature); \
	iterator(ctx, kernelStruct.txm_code_signature.fork_trust_pair); \
	\
	iterator(ctx, kernelStruct.pmap_cs_region.pmap_cs_region_next); \
	iterator(ctx, kernelStruct.pmap_cs_region.pmap_cs_region_right); \
	...
	iterator(ctx, kernelStruct.pmap_cs_code_directory.signing_identifier); \
	iterator(ctx, kernelStruct.pmap_cs_code_directory.team_identifier); \
...
#define RELAXIN_KERNEL_SYMBOLS_ITERATE(ctx, iterator) \
	iterator(ctx, relaxinKernelSymbol.sptm_args);
...
#define SYSTEM_INFO_ITERATE(ctx, iterator) \
	...
	RELAXIN_KERNEL_SYMBOLS_ITERATE(ctx, iterator);

__attribute__((__unused__)) static void _safe_xpc_dictionary_get_string(xpc_object_t xdict,
                                                                        const char *name,
                                                                        char **out) {
...
__attribute__((__unused__)) static void _safe_xpc_dictionary_set_string(xpc_object_t xdict,
                                                                        const char *name,
                                                                        const char *string) {
    if (string) {
...
#define rlx_ksymbol(name) (gSystemInfo.relaxinKernelSymbol.name ? (gSystemInfo.kernelConstant.slide + gSystemInfo.relaxinKernelSymbol.name) : 0)
