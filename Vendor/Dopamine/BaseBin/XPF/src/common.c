...
static uint64_t xpf_find_PT_INDEX_MAX(void) {
    PFSection *textSection = (gXPF.kernelIsArm64e && !gXPF.isSPTMDevice) ? gXPF.kernelPPLTextSection
...
static uint64_t xpf_find_kfree_data_external(void) {
    ...
    uint64_t kfree_data_external_pre = pfsec_find_prev_inst(sec,
                                                            fatal_error_fmt_end,
                                                            25,
                                                            movW1_0x400_Inst,
    ...
    int decRet = arm64_dec_b_l(pfsec_read32(sec, kfree_data_external_pre + 4),
                               kfree_data_external_pre + 4,
                               &kfree_data_external,
                               NULL);
    XPF_ASSERT(decRet == 0);
...
static bool xpf_register_equal(arm64_register left, arm64_register right) {
    return ARM64_REG_GET_TYPE(left) == ARM64_REG_GET_TYPE(right) && ARM64_REG_GET_NUM(left) == ARM64_REG_GET_NUM(right);
}

static bool xpf_decode_boot_tc_gate(uint64_t compareAddress, uint16_t expectedType, uint64_t *gateAddressOut) {
    PFSection *section = gXPF.kernelTextSection;
    arm64_register compareDestination = {0};
    arm64_register compareSource = {0};
    uint16_t compareImmediate = 0;
    bool setsFlags = false;
    if (arm64_dec_sub_imm(pfsec_read32(section, compareAddress),
                          &compareDestination,
                          &compareSource,
                          &compareImmediate,
                          &setsFlags)
            != 0
        || !setsFlags || !ARM64_REG_IS_W(compareDestination)
        || ARM64_REG_GET_NUM(compareDestination) != ARM64_REG_NUM_ZR || !ARM64_REG_IS_W(compareSource)
        || compareImmediate != expectedType) {
        return false;
    }
    if (arm64_dec_b_c_cond(pfsec_read32(section, compareAddress + 4), compareAddress + 4, NULL, NULL, NULL) != 0) {
        return false;
    }

    arm64_register pageRegister = {0};
    bool isAdrp = false;
    if (arm64_dec_adr_p(pfsec_read32(section, compareAddress + 8), compareAddress + 8, NULL, &pageRegister, &isAdrp)
            != 0
        || !isAdrp) {
        return false;
    }

    arm64_register loadRegister = {0};
    arm64_register loadBase = {0};
    char loadType = 0;
    if (arm64_dec_ldr_imm(pfsec_read32(section, compareAddress + 12), &loadRegister, &loadBase, NULL, &loadType, NULL)
            != 0
        || loadType != 'b' || !xpf_register_equal(pageRegister, loadBase)) {
        return false;
    }

    bool isTbnz = false;
    arm64_register testRegister = {0};
    uint64_t testBit = UINT64_MAX;
    if (arm64_dec_tb_n_z(pfsec_read32(section, compareAddress + 16),
                         compareAddress + 16,
                         &isTbnz,
                         &testRegister,
                         NULL,
                         &testBit)
            != 0
        || !isTbnz || testBit != 0 || !xpf_register_equal(loadRegister, testRegister)) {
        return false;
    }

    uint64_t gateAddress = pfsec_arm64_resolve_adrp_ldr_str_add_reference_auto(section, compareAddress + 12);
    if (!gateAddress)
        return false;
    *gateAddressOut = gateAddress;
    return true;
}

static uint64_t xpf_find_boot_tc_loaded(void *context) {
    uint32_t requestedIndex = (uint32_t)(uintptr_t)context;
    XPF_ASSERT(requestedIndex == 1 || requestedIndex == 2);

    __block struct {
        uint64_t addresses[8];
        size_t count;
    } entitlementStrings = {0};
    PFStringMetric *stringMetric = pfmetric_string_init("com.apple.private.pmap.load-trust-cache");
    pfmetric_run(gXPF.kernelStringSection, stringMetric, ^(uint64_t address, bool *stop) {
        (void)stop;
        if (entitlementStrings.count < sizeof(entitlementStrings.addresses) / sizeof(entitlementStrings.addresses[0])) {
            entitlementStrings.addresses[entitlementStrings.count++] = address;
        }
    });
    pfmetric_free(stringMetric);
    XPF_ASSERT(entitlementStrings.count > 0);

    __block struct {
        uint64_t addresses[16];
        size_t count;
    } functionStarts = {0};
    for (size_t stringIndex = 0; stringIndex < entitlementStrings.count; stringIndex++) {
        PFXrefMetric *xrefMetric = pfmetric_xref_init(entitlementStrings.addresses[stringIndex],
                                                      XREF_TYPE_MASK_REFERENCE);
        pfmetric_run(gXPF.kernelTextSection, xrefMetric, ^(uint64_t address, bool *stop) {
            (void)stop;
            uint64_t functionStart = pfsec_find_function_start(gXPF.kernelTextSection, address);
            for (size_t index = 0; index < functionStarts.count; index++) {
                if (functionStarts.addresses[index] == functionStart)
                    return;
            }
            if (functionStart
                && functionStarts.count < sizeof(functionStarts.addresses) / sizeof(functionStarts.addresses[0])) {
                functionStarts.addresses[functionStarts.count++] = functionStart;
            }
        });
        pfmetric_free(xrefMetric);
    }
    XPF_ASSERT(functionStarts.count > 0);

    uint64_t matchedOSGate = 0;
    uint64_t matchedAppGate = 0;
    size_t matchCount = 0;
    for (size_t functionIndex = 0; functionIndex < functionStarts.count; functionIndex++) {
        uint64_t functionStart = functionStarts.addresses[functionIndex];
        uint64_t functionEnd = functionStart + 0x800;
        uint64_t sectionEnd = gXPF.kernelTextSection->info.vmaddr + gXPF.kernelTextSection->info.size;
        if (functionEnd > sectionEnd)
            functionEnd = sectionEnd;
        for (uint64_t address = functionStart; address + 44 <= functionEnd; address += 4) {
            uint64_t osGate = 0;
            uint64_t appGate = 0;
            if (!xpf_decode_boot_tc_gate(address, 13, &osGate) || !xpf_decode_boot_tc_gate(address + 24, 14, &appGate)
                || appGate != osGate + 1) {
                continue;
            }
            if (matchCount == 0 || osGate != matchedOSGate || appGate != matchedAppGate) {
                matchedOSGate = osGate;
                matchedAppGate = appGate;
                matchCount++;
            }
        }
    }
    if (matchCount != 1) {
        xpf_set_error("boot trustcache gate semantic match count: %zu", matchCount);
        return 0;
    }
    return requestedIndex == 1 ? matchedOSGate : matchedAppGate;
}
...
static uint64_t xpf_find_task_collect_crash_info(void) {
    ...
    PFXrefMetric *task_collect_crash_infoOutlineMetric = pfmetric_xref_init(task_collect_crash_info,
                                                                            XREF_TYPE_MASK_JUMP);
...
static uint64_t xpf_find_task_itk_space(void) {
    ...
    while (true) {
        ldrAddr = pfsec_find_next_inst(gXPF.kernelTextSection,
                                       ldrAddr,
                                       (ldrEndAddr - ldrAddr) / 4,
                                       ldrAnyInst,
                                       ldrAnyMask);
...
static uint64_t xpf_find_vm_map_pmap(void) {
    ...
    uint32_t mask[2] = {0};
    arm64_gen_ldr_imm(0,
                      gXPF.kernelIsArm64e ? LDR_STR_TYPE_PRE_INDEX : LDR_STR_TYPE_UNSIGNED,
                      ARM64_REG_ANY,
                      ARM64_REG_ANY,
                      OPT_UINT64_NONE,
                      &inst[0],
    ...
    pfmetric_run_in_range(gXPF.kernelTextSection,
                          xrefAddr,
                          xrefAddr - (100 * sizeof(uint32_t)),
                          patternMetric,
                          ^(uint64_t vmaddr, bool *stop) {
                              if (arm64_dec_adr_p(pfsec_read32(gXPF.kernelTextSection, vmaddr - 4),
                                                  vmaddr - 4,
                                                  NULL,
                                                  NULL,
                                                  NULL)
                                  != 0) {
                                  arm64_dec_ldr_imm(pfsec_read32(gXPF.kernelTextSection, vmaddr),
                                                    NULL,
                                                    NULL,
                                                    &vm_map_pmap,
                                                    NULL,
...
static uint64_t xpf_find_proc_struct_size(void) {
    if (strcmp(gXPF.darwinVersion, "22.0.0") >= 0) {
        ...
        arm64_gen_ldr_imm(0,
                          ...
                          &ldrAnyInst,
                          &ldrAnyMask);
        uint64_t ldrAddr = pfsec_find_prev_inst(gXPF.kernelTextSection,
                                                procTaskStringXref,
                                                0x20,
                                                ldrAnyInst,
                                                ldrAnyMask);
        ...
        PFPatternMetric *patternMetric = pfmetric_pattern_init(&procStringAddr,
                                                               &mask,
                                                               sizeof(procStringAddr),
                                                               sizeof(uint64_t));
...
static uint64_t xpf_find_perfmon_devices(void) {
    ...
    uint64_t movAddr = pfsec_find_next_inst(gXPF.kernelTextSection,
                                            perfmon_dev_open,
                                            0,
                                            movWAny_0x28Inst,
                                            movWAny_0x28Mask);
...
static uint64_t xpf_find_cdevsw(void) {
    ...
    uint64_t before_bl_spec_kqfilterAddr = pfsec_find_next_inst(gXPF.kernelTextSection,
                                                                vn_kqfilter,
                                                                100,
                                                                movW2Inst,
                                                                movW2Mask);
    ...
    arm64_dec_b_l(pfsec_read32(gXPF.kernelTextSection, bl_spec_kqfilterAddr),
                  bl_spec_kqfilterAddr,
                  &spec_kqfilter,
                  NULL);
...
static uint64_t xpf_find_proc_apply_sandbox(void) {
    PFSection *textSection = gXPF.kernelSandboxTextSection ?: gXPF.kernelTextSection;
    PFSection *logSection = gXPF.kernelSandboxOSLogSection ?: gXPF.kernelOSLogSection;
    ...
    pfmetric_run(logSection, stringMetric, ^(uint64_t vmaddr, bool *stop) {
    ...
    pfmetric_run(textSection, xrefMetric, ^(uint64_t vmaddr, bool *stop) {
        proc_apply_sandbox = pfsec_find_function_start(textSection, vmaddr);
        *stop = true;
    });
    pfmetric_free(xrefMetric);
...
static uint64_t xpf_find_mac_label_set(void) {
    if (strcmp(gXPF.darwinVersion, "23.0.0") >= 0) {
        PFStringMetric *sentinelMetric = pfmetric_string_init("cannot set mac label to ~0 @%s:%d");
        __block uint64_t sentinelString = 0;
        pfmetric_run(gXPF.kernelStringSection, sentinelMetric, ^(uint64_t vmaddr, bool *stop) {
            sentinelString = vmaddr;
            *stop = true;
        });
        pfmetric_free(sentinelMetric);
        XPF_ASSERT(sentinelString);

        __block uint64_t panicHelper = 0;
        PFXrefMetric *stringXref = pfmetric_xref_init(sentinelString, XREF_TYPE_MASK_REFERENCE);
        pfmetric_run(gXPF.kernelTextSection, stringXref, ^(uint64_t vmaddr, bool *stop) {
            panicHelper = pfsec_find_function_start(gXPF.kernelTextSection, vmaddr);
            *stop = true;
        });
        pfmetric_free(stringXref);
        XPF_ASSERT(panicHelper);

        __block uint64_t macLabelSet = 0;
        PFXrefMetric *callXref = pfmetric_xref_init(panicHelper, XREF_TYPE_MASK_CALL);
        pfmetric_run(gXPF.kernelTextSection, callXref, ^(uint64_t vmaddr, bool *stop) {
            macLabelSet = pfsec_find_function_start(gXPF.kernelTextSection, vmaddr);
            *stop = true;
        });
        pfmetric_free(callXref);
        XPF_ASSERT(macLabelSet);
        return macLabelSet;
    }
    ...
    uint64_t firstBLAddr = pfsec_find_next_inst(textSection, proc_apply_sandbox, 100, blAnyInst, blAnyMask);
    XPF_ASSERT(firstBLAddr);
    uint64_t secondBLAddr = pfsec_find_next_inst(textSection, firstBLAddr + 4, 100, blAnyInst, blAnyMask);
    XPF_ASSERT(secondBLAddr);

    uint64_t mac_label_set = 0;
    arm64_dec_b_l(pfsec_read32(textSection, secondBLAddr), secondBLAddr, &mac_label_set, NULL);
...
static uint64_t xpf_find_nsysent(void) {
    ...
    uint64_t mov1Addr = pfsec_find_next_inst(gXPF.kernelTextSection,
...
static uint64_t xpf_find_mach_kobj_count(void) {
    ...
    uint64_t ldrswAddr = pfsec_find_next_inst(gXPF.kernelTextSection,
                                              proc_get_syscall_filter_mask_size,
                                              40,
                                              ldrswInst,
                                              ldrswMask);
...
static uint64_t xpf_find_developer_mode_enabled(void) {
    ...
    uint32_t adrpAnyInst = 0, adrpAnyMask = 0;
    arm64_gen_adr_p(OPT_BOOL(true), OPT_UINT64_NONE, OPT_UINT64_NONE, ARM64_REG_ANY, &adrpAnyInst, &adrpAnyMask);
    uint64_t branchTarget = pfsec_find_prev_inst(gXPF.kernelTextSection,
                                                 stringXrefAddr - 4,
                                                 20,
                                                 adrpAnyInst,
                                                 adrpAnyMask);
    ...
    if (!adrpAddAddr || ldrLitAddr > adrpAddAddr) {
        arm64_dec_ldr_lit(pfsec_read32(gXPF.kernelTextSection, ldrLitAddr),
                          ldrLitAddr,
                          &developer_mode_candidate,
                          NULL);
    } else {
        developer_mode_candidate = pfsec_arm64_resolve_adrp_ldr_str_add_reference_auto(gXPF.kernelTextSection,
                                                                                       adrpAddAddr + 4);
    }
...
static uint64_t xpf_find_thread_machine_kstackptr(void) {
    ...
    pfmetric_run(gXPF.kernelTextSection, xrefMetric, ^(uint64_t vmaddr, bool *stop) {
        ...
        /* Darwin 23 authenticates the stack pointer with the address of the
		 * machine_kstackptr field, so the load uses a pre-indexed base update. */
        if (!ldrAddr) {
            arm64_gen_ldr_imm(0, LDR_STR_TYPE_PRE_INDEX, ARM64_REG_ANY, threadReg, OPT_UINT64_NONE, &ldrInst, &ldrMask);
            ldrAddr = pfsec_find_prev_inst(gXPF.kernelTextSection, vmaddr, 50, ldrInst, ldrMask);
        }
        if (!ldrAddr)
            return;

        uint32_t readLdrInst = pfsec_read32(gXPF.kernelTextSection, ldrAddr);
        if (arm64_dec_ldr_imm(readLdrInst, NULL, NULL, &machine_kstackptr, NULL, NULL) == 0 && machine_kstackptr)
            *stop = true;
    });
    pfmetric_free(xrefMetric);
...
static uint64_t xpf_find_thread_machine_contextData(void) {
    uint32_t addX1X0 = 0, addX1X0Mask = 0;
    arm64_gen_add_imm(ARM64_REG_X(1), ARM64_REG_ANY, OPT_UINT64_NONE, &addX1X0, &addX1X0Mask);

    uint32_t inst[5] = {
        0xd538d080, // mrs x0, tpidr_el1
        addX1X0,    // add x1, x0, #machine_upcb
        0xf9400020, // ldr x0, [x1]
        0xf2e3d221, // movk x1, #0x1e91, lsl #48
        0xdac11820, // autda x0, x1
    };
    uint32_t mask[5] = {
        0xffffffff,
        addX1X0Mask,
    ...
    PFPatternMetric *patternMetric = pfmetric_pattern_init(inst, mask, sizeof(inst), sizeof(uint32_t));
    pfmetric_run(gXPF.kernelTextSection, patternMetric, ^(uint64_t vmaddr, bool *stop) {
        arm64_register destination = {0};
        arm64_register source = {0};
        uint16_t machineUpcb = 0;
        if (arm64_dec_add_imm(pfsec_read32(gXPF.kernelTextSection, vmaddr + sizeof(uint32_t)),
                              &destination,
                              &source,
                              &machineUpcb)
                == 0
            && ARM64_REG_GET_NUM(destination) == 1 && ARM64_REG_GET_NUM(source) == 0
            && machineUpcb >= sizeof(uint64_t)) {
            /* contextData immediately precedes the authenticated machine_upcb field. */
            machine_contextData = machineUpcb - sizeof(uint64_t);
            *stop = true;
        }
    });
    pfmetric_free(patternMetric);
...
static uint64_t xpf_find_flush_mmu_tlb_region_asid_async(void) {
    uint32_t inst[] = {
        0x34000000, // cbz w?, ...
        0x39400040, // ldrb w?, [x2, #?]
        0x71000c1f, // cmp w?, #3
        0x54000001, // b.ne ...
    ...
    pfmetric_run(gXPF.kernelTextSection, metric, ^(uint64_t vmaddr, bool *stop) {
        /* This helper is a leaf entered through BTI. Function-start heuristics can
		 * merge it with the preceding pmap routine, so anchor on the nearest BTI. */
        uint64_t candidate = pfsec_find_prev_inst(gXPF.kernelTextSection, vmaddr, 0x40, 0xd503245f, 0xffffffff);
        if (candidate && vmaddr - candidate <= 0x40) {
            function = candidate;
            *stop = true;
        }
    });
...
void xpf_common_init(void) {
    ...
    xpf_item_register("kernelSymbol.boot_os_tc_loaded", xpf_find_boot_tc_loaded, (void *)(uintptr_t)1);
    xpf_item_register("kernelSymbol.boot_app_tc_loaded", xpf_find_boot_tc_loaded, (void *)(uintptr_t)2);
    ...
    xpf_item_register("kernelSymbol.proc_get_syscall_filter_mask_size",
                      xpf_find_proc_get_syscall_filter_mask_size,
                      NULL);
    ...
    xpf_item_register("kernelSymbol.flush_mmu_tlb_region_asid_async", xpf_find_flush_mmu_tlb_region_asid_async, NULL);
