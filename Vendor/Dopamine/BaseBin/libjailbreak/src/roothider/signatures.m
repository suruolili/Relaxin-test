...
MachO *fat_find_preferred_slice(Fat *fat) {
    ...
    if (host_get_cpu_information(&cputype, &cpusubtype) != 0) {
        return NULL;
    }
    ...
    if (cpusubtype == CPU_SUBTYPE_ARM64E) {
        ...
        if (!candidateSlice) {
            ...
            if (candidateSlice) {
                ...
                if (macho_get_filetype(candidateSlice) == MH_EXECUTE)
                    candidateSlice = NULL;
            }
        }
    }
...
NSString *resolveLoaderExecutablePaths(NSString *loadPath, NSString *loaderPath, NSString *mainExecutablePath) {
    if ([loadPath hasPrefix:@"@loader_path/"] || [loadPath isEqualToString:@"@loader_path"]) {
        return [loadPath stringByReplacingCharactersInRange:NSMakeRange(0, sizeof("@loader_path") - 1)
                                                 withString:loaderPath.stringByDeletingLastPathComponent];
    }
    if ([loadPath hasPrefix:@"@executable_path/"] || [loadPath isEqualToString:@"@executable_path"]) {
        return [loadPath stringByReplacingCharactersInRange:NSMakeRange(0, sizeof("@executable_path") - 1)
                                                 withString:mainExecutablePath.stringByDeletingLastPathComponent];
    }
    return nil;
};
            ...
            if (fat) {
                ...
                if (macho) {
                    macho_enumerate_rpaths(macho, ^(const char *rpathCStr, bool *stop) {
                        NSString *possiblePath = [loadPath
                            stringByReplacingCharactersInRange:NSMakeRange(0, sizeof("@rpath") - 1)
                                                    withString:@(rpathCStr)];
                        possiblePath = resolveLoaderExecutablePaths(possiblePath, loaderPath, mainExecutablePath)
                            ?: possiblePath;
...
NSString *resolveLoadPath(NSString *loadPath,
                          NSString *loaderPath,
...
static void recurse_handler(NSString *loadPath,
                            NSString *loaderPath,
                            NSString *mainExecutablePath,
                            NSString *workingDir,
                            NSMutableArray *fileCaches,
                            NSMutableArray *rpathStack,
    ...
    @autoreleasepool {
        ...
        if (!resolvedLoadPath) {
            JBLogError("Failed to resolve dependency library for %s (loader: %s, mainExecutable: %s)",
                       loadPath.fileSystemRepresentation,
                       loaderPath.fileSystemRepresentation,
                       mainExecutablePath.fileSystemRepresentation);
            return;
        }
        ...
        if (fd < 0) {
            JBLogError("Failed to open binary at path: %s (loader: %s)",
                       resolvedLoadPath.fileSystemRepresentation,
                       loaderPath.fileSystemRepresentation);
            return;
        }
        ...
        if (macho_calculate_adhoc_cdhash(macho, cdhash)) {
            isAdhocSigned = true;
            int ret = ensure_randomized_cdhash_for_slice(realLoadPath.fileSystemRepresentation,
                                                         macho->archDescriptor.offset,
                                                         cdhash);
            if (ret == 0) {
                cdhashWasKnown = cdhashesContains(cdhash);
                if (!cdhashWasKnown) {
                    if (!is_cdhash_trustcached(cdhash)) {
                        // If something is trustcached we do not want to add it to your array.
                        // Dependencies still need parsing because one may have changed.
                        cdhashesAdd(cdhash);
                    }
                }
            } else {
                cdhashWasKnown = false;
                JBLogError("ensure_randomized_cdhash_for_slice(%llx) failed: %s -> (%d)",
                           macho->archDescriptor.offset,
                           realLoadPath.fileSystemRepresentation,
                           ret);
            }
        }
