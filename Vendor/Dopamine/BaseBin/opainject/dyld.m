...
void iterateImages(task_t task,
                   vm_address_t imageStartPtr,
                   void (^iterateBlock)(char *, struct dyld_image_info *, BOOL *)) {
    ...
    for (int i = 0; i < imageInfos.infoArrayCount; i++) {
        @autoreleasepool {
            ...
            if (task_read(task,
                          (vm_address_t)infoArray[i].imageFilePath,
                          &currentImagePath[0],
                          sizeof(currentImagePath))
                == KERN_SUCCESS) {
                BOOL stop = NO;
...
void iterateLoadCommands(task_t task,
                         vm_address_t imageAddress,
                         void (^iterateBlock)(const struct load_command *cmd, vm_address_t cmdAddress, BOOL *stop)) {
...
void iterateSections(task_t task,
                     vm_address_t commandAddress,
                     const struct segment_command_64 *segmentCommand,
                     void (^iterateBlock)(const struct section_64 *, BOOL *)) {
...
void iterateSymbols(task_t task,
                    vm_address_t imageAddress,
                    void (^iterateBlock)(const char *, const char *, vm_address_t, BOOL *)) {
...
vm_address_t remoteDlSymFindImage(task_t task, vm_address_t allImageInfoAddr, const char *symbolName, char **imageOut) {
    ...
    iterateImages(task, allImageInfoAddr, ^(char *imageFilePath, struct dyld_image_info *imageInfo, BOOL *stop1) {
        stop1_b = stop1;
        iterateSymbols(task,
                       (vm_address_t)imageInfo->imageLoadAddress,
                       ^(const char *iterSymbolName, const char *type, vm_address_t value, BOOL *stop2) {
                           if (strcmp(type, "N_SECT") == 0) {
                               if (strcmp(iterSymbolName, symbolName) == 0) {
                                   outAddr = value;
                                   if (imageOut) {
...
vm_address_t scanTextSegmentForMemory(task_t task,
                                      vm_address_t commandAddress,
                                      const struct segment_command_64 *textCmd,
                                      vm_address_t slide,
                                      char *memory,
                                      size_t memorySize,
                                      int alignment) {
...
vm_address_t scanLibrariesForMemory(task_t task,
                                    vm_address_t imageStartPtr,
                                    char *memory,
                                    size_t memorySize,
                                    int alignment) {
    ...
    iterateImages(task, imageStartPtr, ^(char *imageFilePath, struct dyld_image_info *imageInfo, BOOL *stopImages) {
        ...
        __block BOOL firstSegmentCommand = YES;
        //printf("- iterating %s -\n", imageFilePath);
        iterateLoadCommands(task,
                            (vm_address_t)imageInfo->imageLoadAddress,
                            ^(const struct load_command *cmd, vm_address_t cmdAddress, BOOL *stopCommands) {
                                if (cmd->cmd == LC_SEGMENT_64) {
                                    ...
                                    if (strncmp("__TEXT", segmentCommand.segname, 16) == 0) {
                                        vm_address_t addrIfFound = scanTextSegmentForMemory(task,
                                                                                            cmdAddress,
                                                                                            &segmentCommand,
                                                                                            slide,
                                                                                            memory,
