...
void __dsc_file_enumerate_mappings(int fd,
                                   struct dyld_cache_header *header,
                                   uintptr_t slide,
                                   bool (*enumeratorFunc)(int fd,
                                                          struct dyld_cache_header *header,
                                                          uintptr_t slide,
    ...
    if (read(fd, mappingInfos, sizeof(struct dyld_cache_mapping_info) * header->mappingCount)
        != sizeof(struct dyld_cache_mapping_info) * header->mappingCount)
        return;
...
void __dsc_enumerate_mappings(uintptr_t slide,
                              bool (*enumeratorFunc)(int fd,
                                                     struct dyld_cache_header *header,
                                                     uintptr_t slide,
                                                     struct dyld_cache_mapping_info *mapping)) {
    ...
    struct dyld_cache_header header;
    if (read(dscFd, &header, sizeof(header)) != sizeof(header)) {
        close(dscFd);
        return;
    }
    ...
    if (read(dscFd, subcacheEntries, sizeof(struct dyld_subcache_entry_v1) * header.subCacheArrayCount)
        != sizeof(struct dyld_subcache_entry_v1) * header.subCacheArrayCount) {
        close(dscFd);
        return;
    };
    ...
    for (uint32_t i = 0; i < header.subCacheArrayCount; i++) {
        ...
        if (subcacheFd >= 0) {
            ...
            if (read(subcacheFd, &subcacheHeader, sizeof(subcacheHeader)) != sizeof(subcacheHeader))
                continue;
...
bool __dsc_mapping_make_private(int fd,
                                struct dyld_cache_header *header,
                                uintptr_t slide,
                                struct dyld_cache_mapping_info *mapping) {
    if (mapping->initProt & PROT_EXEC) {
        ...
        if (r == 0) {
            void *r = mmap((void *)(mapping->address + slide),
                           mapping->size,
                           PROT_READ | PROT_EXEC,
...
extern bool ORIG(_ZN5dyld313loadDyldCacheERKNS_18SharedCacheOptionsEPNS_19SharedCacheLoadInfoE)(uintptr_t options,
                                                                                                uintptr_t results);
...
__attribute__((
    optimize("O0"),
    noinline)) bool ORIG__ZN5dyld313loadDyldCacheERKNS_18SharedCacheOptionsEPNS_19SharedCacheLoadInfoE(uintptr_t
                                                                                                           options,
                                                                                                       uintptr_t
                                                                                                           results) {
    __asm("\
...
bool HOOK(_ZN5dyld313loadDyldCacheERKNS_18SharedCacheOptionsEPNS_19SharedCacheLoadInfoE)(uintptr_t options,
                                                                                         uintptr_t results) {
