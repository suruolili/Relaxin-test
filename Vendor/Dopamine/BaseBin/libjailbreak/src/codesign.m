...
typedef CF_OPTIONS(uint32_t, SecCSFlags) {
    ...
    kSecCSCheckTrustedAnchors = 1
        << 27, /* build certificate chain to system trust anchors, not to any self-signed certificate */
    kSecCSQuickCheck = 1 << 26, /* (internal) */
    kSecCSApplyEmbeddedPolicy = 1
        << 25, /* Apply Embedded (iPhone) policy regardless of the platform we're running on */
};
...
int resign_file(NSString *filePath, NSString *identifier, bool preserveMetadata) {
    ...
    if (preserveMetadata) {
        parameters[(__bridge NSString *)kSecCodeSignerPreserveMetadata] = @(
            kSecCSPreserveIdentifier | kSecCSPreserveRequirements | kSecCSPreserveEntitlements
    ...
    if (status == 0) {
        SecStaticCodeRef code;
        status = SecStaticCodeCreateWithPathAndAttributes((__bridge CFURLRef)[NSURL fileURLWithPath:filePath],
                                                          kSecCSDefaultFlags,
                                                          NULL,
                                                          &code);
        if (status == 0) {
            ...
            if (status == 0) {
                ...
                if (status == 0) {
                    ...
                    JBLogError("SecCodeCopySigningInformation failed: %s (%d)",
                               ((__bridge NSString *)SecCopyErrorMessageString(status, NULL)).UTF8String,
                               status);
                    retval = 203;
                }
            }
            ...
            JBLogError("SecStaticCodeCreateWithPathAndAttributes failed: %s (%d)",
                       ((__bridge NSString *)SecCopyErrorMessageString(status, NULL)).UTF8String,
                       status);
            retval = 202;
        }
        ...
        JBLogError("SecCodeSignerCreate failed: %s (%d)",
                   ((__bridge NSString *)SecCopyErrorMessageString(status, NULL)).UTF8String,
                   status);
        retval = 201;
    }
