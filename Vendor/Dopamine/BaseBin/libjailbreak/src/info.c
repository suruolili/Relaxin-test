...
void jbinfo_initialize_hardcoded_offsets(void) {
    ...
    gSystemInfo.kernelStruct.pmap.txm_addr_space = 0;
    ...
    gSystemInfo.kernelStruct.pmap_cs_region.pmap_cs_region_right = 0x8;
    ...
    gSystemInfo.kernelStruct.pmap_cs_code_directory.signing_identifier = 0x0;
    gSystemInfo.kernelStruct.pmap_cs_code_directory.team_identifier = 0x0;
    ...
    // label
    gSystemInfo.kernelStruct.label.sandbox = 0x10;
        ...
        if (strcmp(darwinVersion, "21.2.0") >= 0) { // iOS 15.2+
            ...
            if (strcmp(darwinVersion, "21.4.0") >= 0) { // iOS 15.4+
                ...
                if (strcmp(darwinVersion, "22.0.0") >= 0) { // iOS 16+
...
#ifdef __arm64e__
                    // pmap_cs_code_directory
                    gSystemInfo.kernelStruct.pmap_cs_code_directory.signing_identifier = 0x180;
                    gSystemInfo.kernelStruct.pmap_cs_code_directory.team_identifier = 0x188;
                    ...
                    if (strcmp(darwinVersion, "22.1.0") >= 0
                        && (xnuMajor > 8792
                            || (xnuMajor == 8792
                                && xnuMinor
                                    >= 42))) { // iOS 16.1+ (Exluding 16.1b1 - 16.1b3 on iOS and 16.1b1 - 16.1b4 on iPadOS)
                        ...
                        if (strcmp(darwinVersion, "22.3.0") >= 0) { // iOS 16.3+
                            ...
                            if (strcmp(darwinVersion, "22.4.0") >= 0) { // iOS 16.4+
                                ...
                                if (strcmp(darwinVersion, "23.0.0") >= 0) { // iOS 17+
                                    ...
                                    if (strcmp(darwinVersion, "24.0.0") < 0 && xnuMajor == 10002) {
                                        gSystemInfo.kernelStruct.pmap.txm_addr_space = 0xB0;
                                        gSystemInfo.kernelStruct.pmap.txm_trust_level = 0xB8;
                                        gSystemInfo.kernelStruct.txm_address_space.allows_invalid_code = 0x20;
                                        gSystemInfo.kernelStruct.txm_address_space.main_region = 0x40;
                                        gSystemInfo.kernelStruct.txm_region.code_signature = 0x20;
                                        gSystemInfo.kernelStruct.txm_code_signature.fork_trust_pair = 0xC8;
                                    }
                                        ...
                                        if (strcmp(darwinVersion, "23.4.0") >= 0) { // iOS 17.4+
                                            ...
                                            if (strcmp(darwinVersion, "24.0.0") >= 0) { // iOS 18+
                                                ...
                                                gSystemInfo.kernelStruct.task.task_can_transfer_memory_ownership = 0x568
                                                    + taskJitboxAdjust;
#else
                                                gSystemInfo.kernelStruct.task
                                                    .task_can_transfer_memory_ownership = 0x548;
#endif
                                                if (strcmp(darwinVersion, "24.1.0") >= 0) { // iOS 18.1+
                                                    ...
                                                    if (strcmp(darwinVersion, "24.4.0") >= 0) { // iOS 18.4+
                                                        gSystemInfo.kernelConstant
                                                            .PVH_HIGH_FLAGS = 0x7F90000000000000LL;

                                                        // task
#ifdef __arm64e__
                                                        gSystemInfo.kernelStruct.task
                                                            .task_can_transfer_memory_ownership = 0x588
                                                            + taskJitboxAdjust;
#else
                                                        gSystemInfo.kernelStruct.task
                                                            .task_can_transfer_memory_ownership = 0x568;
#endif
...
uint64_t get_vm_real_kernel_page_shift(void) {
    return vm_kernel_page_shift;
}

uint64_t get_l1_block_size(void) {
    return vm_real_kernel_page_size == 0x4000 ? 0x1000000000 : 0;
}

uint64_t get_l1_block_mask(void) {
    uint64_t size = get_l1_block_size();
    return size ? size - 1 : 0;
}

uint64_t get_l1_block_count(void) {
    return vm_real_kernel_page_size == 0x4000 ? 8 : 0;
}

uint64_t get_l2_block_size(void) {
    return vm_real_kernel_page_size == 0x4000 ? 0x2000000 : 0;
}

uint64_t get_l2_block_mask(void) {
    uint64_t size = get_l2_block_size();
    return size ? size - 1 : 0;
}

uint64_t get_l2_block_count(void) {
    return vm_real_kernel_page_size == 0x4000 ? 2048 : 0;
}
