...
int64_t sandbox_extension_consume(const char *extension_token) {
    ...
    if (!strcmp(extension_token, "invalid"))
        return 0;
...
__options_decl(mach_msg_option64_t,
               uint64_t,
               {
...
mach_msg_return_t mach_msg2_internal(void *data,
...
mach_msg_return_t mach_msg2(void *data,
                            mach_msg_option64_t option64,
...
#define MACH_MSG2_SHIFT_ARGS(lo, hi) ((uint64_t)hi << 32 | (uint32_t)lo)
    return mach_msg2_internal(data,
                              option64,
                              ...
                              MACH_MSG2_SHIFT_ARGS(rcv_size, priority),
                              timeout);
#undef MACH_MSG2_SHIFT_ARGS
}

kern_return_t mach_msg(mach_msg_header_t *msg,
...
kern_return_t _kernelrpc_mach_ports_lookup3(task_t target_task,
                                            mach_port_t *port1,
                                            mach_port_t *port2,
                                            mach_port_t *port3);
