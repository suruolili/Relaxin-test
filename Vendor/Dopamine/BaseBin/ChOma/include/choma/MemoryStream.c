...
int memory_stream_insert(MemoryStream *stream, uint64_t offset, size_t size, const void *inBuf) {
    ...
    if (memory_stream_expand(stream, 0, size) != 0)
        goto fail;
    if (memory_stream_copy_data(stream, offset, stream, offset + size, streamSize - offset) != 0)
        goto fail;
    if (memory_stream_write(stream, offset, size, inBuf) != 0)
        goto fail;
    return 0;

fail:
    fprintf(stderr, "Error: memory_stream_insert failed\n");
    return -1;
}
...
int memory_stream_delete(MemoryStream *stream, uint64_t offset, size_t size) {
    ...
    size_t streamSize = memory_stream_get_size(stream);
    if (memory_stream_copy_data(stream, offset + size, stream, offset, streamSize - (offset + size)) != 0)
        goto fail;
    if (memory_stream_trim(stream, 0, size) != 0)
        goto fail;
    return 0;

fail:
    fprintf(stderr, "Error: memory_stream_delete failed\n");
    return -1;
}
...
size_t memory_stream_get_size(MemoryStream *stream) {
    if (stream->getSize) {
        size_t sizeOut;
        if (stream->getSize(stream, &sizeOut) == 0)
            return sizeOut;
    }
...
int memory_stream_copy_data(MemoryStream *originStream,
    ...
    if (originSize == MEMORY_STREAM_SIZE_INVALID || targetSize == MEMORY_STREAM_SIZE_INVALID) {
        fprintf(stderr, "Error: memory_stream_copy_data failed, invalid size detected\n");
        return -1;
    }

    if (originOffset + size > originSize) {
        fprintf(stderr, "Error: memory_stream_copy_data failed, originOffset OOB\n");
        return -1;
    }
    if (targetOffset + size > targetSize && !(memory_stream_get_flags(targetStream) | MEMORY_STREAM_FLAG_AUTO_EXPAND)) {
        fprintf(stderr, "Error: memory_stream_copy_data failed, targetOffset OOB\n");
        return -1;
    }
    ...
    for (uint32_t copiedSize = 0; copiedSize < size; copiedSize += COPY_DATA_BUFFER_SIZE) {
        ...
        uint64_t readOffset = backwards ? ((originOffset + (size - copiedSize)) - sizeToCopy)
                                        : (originOffset + copiedSize);
        uint64_t writeOffset = backwards ? ((targetOffset + (size - copiedSize)) - sizeToCopy)
                                         : (targetOffset + copiedSize);

        int rr = memory_stream_read(originStream, readOffset, sizeToCopy, buffer);
        if (rr != 0) {
            fprintf(stderr, "Error: memory_stream_copy_data failed on memory_stream_read (%d)\n", rr);
            return rr;
        }
        int wr = memory_stream_write(targetStream, writeOffset, sizeToCopy, buffer);
        if (wr != 0) {
            fprintf(stderr, "Error: memory_stream_copy_data failed on memory_stream_write (%d)\n", wr);
            return rr;
        }
    }
...
int memory_stream_find_memory(MemoryStream *stream,
