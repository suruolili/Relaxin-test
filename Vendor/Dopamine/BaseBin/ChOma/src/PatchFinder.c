...
int raw_buffer_find_memory(uint8_t *buf,
                           uint64_t searchStartOffset,
                           uint64_t searchEndOffset,
                           void *bytes,
                           void *mask,
                           size_t nbytes,
                           uint16_t alignment,
                           uint64_t *foundOffsetOut) {
    __block int r = -1;
...
PFSection *pfsec_init_from_macho(MachO *macho, const char *filesetEntryId, const char *segName, const char *sectName) {
    ...
    if (pfSection) {
        ...
        memory_stream_trim(stream,
                           pfSection->info.fileoff,
                           memory_stream_get_size(stream) - (pfSection->info.fileoff + pfSection->info.size));
...
void pfsec_set_pointer_decoder(PFSection *section,
                               uint64_t (*pointerDecoder)(struct s_PFSection *section,
                                                          uint64_t vmaddr,
                                                          uint64_t value)) {
...
int pfsec_read_at_address(PFSection *section, uint64_t vmaddr, void *outBuf, size_t size) {
    if (vmaddr < section->info.vmaddr)
        return -1;
    if (vmaddr + size > section->info.vmaddr + section->info.size)
        return -1;
...
int pfsec_set_cached(PFSection *section, bool cached) {
    ...
    if (cached != isCachedAlready && section->stream) {
        if (cached) {
            ...
            if (pfsec_get_raw_pointer(section))
                return 0;
...
int pfsec_find_memory_rel(PFSection *section,
                          uint64_t searchStartOffset,
                          uint64_t searchEndOffset,
                          void *bytes,
                          void *mask,
                          size_t nbytes,
                          uint16_t alignment,
                          uint64_t *foundRelOffsetOut) {
    void *rawPtr = pfsec_get_raw_pointer(section);
    if (rawPtr) {
        return raw_buffer_find_memory(rawPtr,
                                      searchStartOffset,
                                      searchEndOffset,
                                      bytes,
                                      mask,
        ...
        // XXX: This does not work for MachOs that are inside the DSC
        // This code path will never be used for that however, since all DSC MachOs have a raw pointer
        return memory_stream_find_memory(section->stream,
                                         searchStartOffset,
                                         searchEndOffset,
                                         bytes,
                                         mask,
...
int pfsec_find_memory(PFSection *section,
                      uint64_t searchStartAddr,
                      uint64_t searchEndAddr,
                      void *bytes,
                      void *mask,
                      size_t nbytes,
                      uint16_t alignment,
                      uint64_t *foundAddrOut) {
    // Ensure searchStartAddr and searchEndAddr stay within section bounds
    if (searchStartAddr < section->info.vmaddr)
        searchStartAddr = section->info.vmaddr;
    if (searchEndAddr < section->info.vmaddr)
        searchEndAddr = section->info.vmaddr;
    if (searchStartAddr > (section->info.vmaddr + section->info.size))
        searchStartAddr = section->info.vmaddr + section->info.size;
    if (searchEndAddr > (section->info.vmaddr + section->info.size))
        searchEndAddr = section->info.vmaddr + section->info.size;

    uint64_t foundRelOff = 0;
    int r = pfsec_find_memory_rel(section,
                                  searchStartAddr - section->info.vmaddr,
                                  searchEndAddr - section->info.vmaddr,
                                  bytes,
                                  mask,
...
uint64_t pfsec_find_prev_inst(PFSection *section,
                              uint64_t startAddr,
...
uint64_t pfsec_find_next_inst(PFSection *section,
                              uint64_t startAddr,
                              uint32_t searchCount,
                              uint32_t inst,
                              uint32_t mask) {
    uint64_t out = 0;
    uint64_t endAddr = searchCount ? (startAddr + (sizeof(uint32_t) * searchCount))
                                   : (section->info.vmaddr + section->info.size);
    pfsec_find_memory(section, startAddr, endAddr, &inst, &mask, sizeof(inst), sizeof(uint32_t), &out);
    if (!out)
        return 0;
    return out;
...
uint64_t pfsec_find_function_start(PFSection *section, uint64_t midAddr) {
    ...
    if (macho) {
        ...
        if (macho_enumerate_function_starts(macho,
                                            ^(uint64_t funcAddr, bool *stop) {
                                                if (funcAddr <= midAddr) {
                                                    start = funcAddr;
                                                } else {
                                                    *stop = true;
    ...
    if (cputype == CPU_TYPE_ARM64) {
        if ((cpusubtype & ~CPU_SUBTYPE_MASK) == CPU_SUBTYPE_ARM64E) {
            ...
            if (frameAddr) {
                uint64_t start = pfsec_find_prev_inst(section,
                                                      frameAddr,
                                                      10,
                                                      0x29a003e0,
                                                      0x3be003e0); // stp ?, ?, [sp, ?]!
...
void _pfsec_run_pattern_metric(PFSection *section,
                               uint64_t startAddr,
                               uint64_t endAddr,
                               PFPatternMetric *patternMetric,
                               void (^matchBlock)(uint64_t vmaddr, bool *stop)) {
    uint16_t alignment = patternMetric->alignment;

    while (pfsec_find_memory(section,
                             startAddr,
                             endAddr,
                             patternMetric->bytes,
                             patternMetric->mask,
                             patternMetric->nbytes,
                             alignment,
                             &startAddr)
           == 0) {
        bool stop = false;
...
void _pfsec_run_string_metric(PFSection *section,
                              uint64_t startAddr,
                              uint64_t endAddr,
                              PFStringMetric *stringMetric,
                              void (^matchBlock)(uint64_t vmaddr, bool *stop)) {
    char *str = NULL;
...
void _pfsec_run_arm64_xref_metric(PFSection *section,
                                  uint64_t startAddr,
                                  uint64_t endAddr,
                                  PFXrefMetric *metric,
                                  void (^matchBlock)(uint64_t vmaddr, bool *stop)) {
    Arm64XrefTypeMask arm64Types = 0;
    if (metric->typeMask == 0)
        return;
    ...
    pfsec_arm64_enumerate_xrefs(section,
                                arm64Types,
                                ^(Arm64XrefType type, uint64_t source, uint64_t target, bool *stop) {
                                    bool match = false;
...
void _pfsec_run_xref_metric(PFSection *section,
                            uint64_t startAddr,
                            uint64_t endAddr,
                            PFXrefMetric *xrefMetric,
                            void (^matchBlock)(uint64_t vmaddr, bool *stop)) {
    uint32_t cputype = 0;
    MachO *macho = pfsec_get_macho(section);
    if (macho) {
...
PFXrefMetric *pfmetric_dynamic_xref_init(
    bool (*dynamicHandler)(PFSection *section, PFXrefMetric *metric, uint64_t location, uint64_t target),
    void *ctx,
    PFXrefTypeMask types) {
    PFXrefMetric *metric = malloc(sizeof(PFXrefMetric));
    if (!metric)
        return NULL;
    memset(metric, 0, sizeof(PFXrefMetric));
...
void pfmetric_run_in_range(PFSection *section,
                           uint64_t startAddr,
                           uint64_t endAddr,
                           void *metric,
                           void (^matchBlock)(uint64_t vmaddr, bool *stop)) {
    if (startAddr == -1ULL)
        startAddr = section->info.vmaddr;
    if (endAddr == -1ULL)
        endAddr = section->info.vmaddr + section->info.size;
