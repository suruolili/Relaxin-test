...
bool pfsec_arm64_scan_register_write(PFSection *section, arm64_register reg, uint64_t endAddr, uint64_t startAddr) {
    if (startAddr == endAddr) {
        ...
        fprintf(stderr, "Fatal error pfsec_arm64_scan_register_write: startAddr > endAddr\n");
        return false;
    }
    ...
    for (uint64_t curAddr = (endAddr - sizeof(uint32_t)); curAddr > startAddr; curAddr -= sizeof(uint32_t)) {
        ...
        if (arm64_dec_add_imm(inst, &destinationReg, &sourceReg, NULL) == 0)
            goto found_inst_match;
        // Check for immediate mov writing to reg
        else if (arm64_dec_mov_imm(inst, &destinationReg, NULL, NULL, NULL) == 0)
            goto found_inst_match;
        // Check for register mov writing to reg
        else if (arm64_dec_mov_reg(inst, &destinationReg, &sourceReg) == 0)
            goto found_inst_match;
        // Check for immediate ldr writing to reg
        else if (arm64_dec_ldr_imm(inst, &destinationReg, NULL, NULL, NULL, NULL) == 0)
            goto found_inst_match;
        // Check for literal ldr writing to reg
        else if (arm64_dec_ldr_lit(inst, curAddr, NULL, &destinationReg) == 0)
            goto found_inst_match;
        // Check for immediate ldrs writing to reg
        else if (arm64_dec_ldrs_imm(inst, &destinationReg, NULL, NULL, NULL, NULL) == 0)
            goto found_inst_match;
...
uint64_t pfsec_arm64_resolve_adrp_ldr_str_add_reference_auto(PFSection *section, uint64_t ldrStrAddAddr) {
    ...
    if (!isAdd) {
        ...
        if (searchCount) {
            ...
            uint64_t specialCaseAddAddr = pfsec_find_prev_inst(section,
                                                               ldrStrAddAddr,
                                                               searchCount,
                                                               specialCaseAddInst,
                                                               specialCaseAddMask);
            if (specialCaseAddAddr) {
                ...
                if (pfsec_arm64_scan_register_write(section, reg, ldrStrAddAddr, specialCaseAddAddr))
                    return 0;
                if (pfsec_arm64_scan_register_write(section, reg, specialCaseAddAddr, adrpAddr))
                    return 0;

                uint64_t baseAddr = pfsec_arm64_resolve_adrp_ldr_str_add_reference(section,
                                                                                   adrpAddr,
                                                                                   specialCaseAddAddr);
                if (!baseAddr)
                    return 0;
                ...
                if (arm64_dec_add_imm(ldrStrAddInst, NULL, NULL, &imm16) != 0) {
                    if (arm64_dec_ldr_imm(ldrStrAddInst, NULL, NULL, &imm64, NULL, NULL) != 0) {
                        if (arm64_dec_str_imm(ldrStrAddInst, NULL, NULL, &imm64, NULL, NULL) != 0) {
                            fprintf(stderr,
                                    "Warning: failed decoding ldrStrAddInst (%#x @ %#llx)\n",
                                    ldrStrAddInst,
                                    ldrStrAddAddr);
                        }
                    }
    ...
    if (pfsec_arm64_scan_register_write(section, reg, ldrStrAddAddr, adrpAddr))
        return 0;
...
uint64_t pfsec_arm64_resolve_stub(PFSection *section, uint64_t stubAddr) {
    ...
    if ((inst[0] & stubMask[0]) == stubInst[0] || (inst[1] & stubMask[1]) == stubInst[1]
...
void pfsec_arm64_enumerate_xrefs(PFSection *section,
                                 Arm64XrefTypeMask types,
                                 void (^xrefBlock)(Arm64XrefType type, uint64_t source, uint64_t target, bool *stop)) {
    bool stop = false;
    if (section->info.initprot & PROT_EXEC) {
        for (uint64_t addr = section->info.vmaddr; addr < (section->info.vmaddr + section->info.size) && !stop;
...
#define ADRP_SEEK_BACK 8
            if (types & ARM64_XREF_TYPE_MASK_ADRP_ADD) {
                ...
                if (arm64_dec_add_imm(inst, &addDestinationReg, &addSourceReg, &addImm) == 0) {
                    uint32_t adrpInst = 0;
                    uint32_t adrpMask = 0;
                    if (arm64_gen_adr_p(OPT_BOOL(true),
                                        OPT_UINT64_NONE,
                                        OPT_UINT64_NONE,
                                        addSourceReg,
                                        &adrpInst,
