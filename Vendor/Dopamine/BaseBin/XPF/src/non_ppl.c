...
static uint64_t xpf_find_trust_cache_rt(void) {
    ...
    return pfsec_read_pointer(gXPF.kernelDataConstSection,
                              pfsec_arm64_resolve_adrp_ldr_str_add_reference_auto(gXPF.kernelTextSection,
                                                                                  adrpAddr + 4));
}
...
void xpf_non_ppl_init(void) {
    if (!gXPF.kernelIsArm64e) {
        ...
        xpf_item_register("kernelSymbol.vm_first_phys",
                          xpf_find_pmap_tt_deallocate_reference,
                          iOS14 ? (void *)(uint32_t)2 : (void *)(uint32_t)1);
        xpf_item_register("kernelSymbol.pv_head_table",
                          xpf_find_pmap_tt_deallocate_reference,
                          iOS14 ? (void *)(uint32_t)1 : (void *)(uint32_t)2);
