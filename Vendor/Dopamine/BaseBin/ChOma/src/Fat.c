...
int fat_parse_slices(Fat *fat) {
    ...
    if (fileSize == MEMORY_STREAM_SIZE_INVALID) {
        fprintf(stderr, "Error: Failed to parse fat slices, memory_stream_get_size returned MEMORY_STREAM_SIZE_INVALID\n");
        return -1;
    }
    ...
    if (fatHeader.magic == FAT_MAGIC || fatHeader.magic == FAT_MAGIC_64) {
        ...
        if (fatHeader.nfat_arch > 5 || fatHeader.nfat_arch < 1) {
            fprintf(stderr, "Error: invalid number of MachO slices (%d), this likely means you are not using an iOS MachO.\n",
                   fatHeader.nfat_arch);
            return -1;
        }
...
Fat *fat_dsc_init_from_memory_stream(MemoryStream *stream,
...
Fat *fat_create_for_macho_array(char *firstInputPath, MachO **machoArray, int machoArrayCount) {
    ...
    for (int i = 1; i < machoArrayCount; i++) {
        if (fat_add_macho(fat, machoArray[i]) != 0) {
            fprintf(stderr, "Error: failed to add MachO to Fat.\n");
            fat_free(fat);
            return NULL;
        }
    }
