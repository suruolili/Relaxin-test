...
static bool _jb_trustcache_legacy_mutation_blocked(const char *operation) {
    if (!trustcache_nokcall_is_required())
        return false;
    JBLogError("trustcache legacy no-kcall mutation blocked operation=%s status=%d", operation, EOPNOTSUPP);
    return true;
}

uint64_t _jb_trustcache_grow(void) {
    if (_jb_trustcache_legacy_mutation_blocked("grow"))
        return 0;
    uint64_t jbTcKern = 0;
...
int jb_trustcache_append_cdhashes(const cdhash_t *hashes, uint32_t hashCount) {
    ...
    if (!hashes)
        return EINVAL;
    if ((size_t)hashCount > SIZE_MAX / sizeof(struct trustcache_entry_v1)) {
        return EOVERFLOW;
    }
    struct trustcache_entry_v1 *entries = malloc((size_t)hashCount * sizeof(*entries));
    if (!entries)
        return ENOMEM;
    ...
    int status = jb_trustcache_append_entries(entries, hashCount);
    free(entries);
    return status;
}

int jb_trustcache_append_entry(struct trustcache_entry_v1 entry) {
    return jb_trustcache_append_entries(&entry, 1);
}

int jb_trustcache_add_entries(struct trustcache_entry_v1 *entries, uint32_t entryCount) {
    return jb_trustcache_append_entries(entries, entryCount);
}

int jb_trustcache_add_cdhashes(cdhash_t *hashes, uint32_t hashCount) {
    return jb_trustcache_append_cdhashes(hashes, hashCount);
}

int jb_trustcache_add_entry(struct trustcache_entry_v1 entry) {
    return jb_trustcache_append_entry(entry);
}
...
void jb_trustcache_debug_print(FILE *f) {
    ...
    _jb_trustcache_enumerate(^(uint64_t jbTcKaddr, bool *stop) {
        ...
        fprintf(f,
                "Jailbreak TrustCache %d <%08x%08x%08x%08x> (length: %u) (kaddr: 0x%llx):\n",
                i++,
                htonl(uuidData[0]),
                htonl(uuidData[1]),
                htonl(uuidData[2]),
    ...
    _trustcache_list_enumerate(^(uint64_t tcKaddr, bool *stop) {
        ...
        uint32_t *uuidData = (uint32_t *)uuid;
        fprintf(f,
                "TrustCache File <%08x%08x%08x%08x> (length: %u) (kaddr: 0x%llx):\n",
                htonl(uuidData[0]),
                htonl(uuidData[1]),
                htonl(uuidData[2]),
                htonl(uuidData[3]),
...
int trustcache_file_upload(trustcache_file_v1 *tc) {
    ...
    if (!tc)
        return EINVAL;
    if ((size_t)tc->length
        > (SIZE_MAX - ksizeof(trustcache) - sizeof(trustcache_file_v1)) / sizeof(trustcache_entry_v1)) {
        return EOVERFLOW;
    }
    ...
    if (existingTcKaddr != 0) {
        ...
        uint64_t prevTcSize = ksizeof(trustcache) + sizeof(trustcache_file_v1)
            + (prevTcLength * sizeof(trustcache_entry_v1));
        if (prevTcSize == tcSize) {
            ...
            usleep(
                10000); // hope for current accesses to finish if there are any (new accesses won't come as we removed the list entry)
...
int trustcache_file_build_from_cdhashes(cdhash_t *CDHashes, uint32_t CDHashCount, trustcache_file_v1 **tcOut) {
    if (!CDHashes || CDHashCount == 0 || !tcOut)
        return -1;
...
int trustcache_query_cdhash(cdhash_t CDHash, bool *foundOut) {
    if (!CDHash || !foundOut)
        return EINVAL;
    if (trustcache_nokcall_is_required()) {
        return trustcache_nokcall_query_cdhash(CDHash, foundOut);
    }
