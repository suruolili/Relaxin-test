...
static uint64_t xpf_find_ppl_dispatch_section(void) {
    ...
    PFPatternMetric *metric = pfmetric_pattern_init(pplCallerInst,
                                                    pplCallerMask,
                                                    sizeof(pplCallerInst),
                                                    sizeof(uint32_t));
...
static uint64_t xpf_find_ppl_enter(void) {
    ...
    arm64_dec_b_l(pfsec_read32(gXPF.kernelTextSection, ppl_dispatch_section + 4),
                  ppl_dispatch_section + 4,
                  &ppl_enter,
                  NULL);
    return ppl_enter;
}
...
static uint64_t xpf_find_ppl_bootstrap_dispatch(void) {
    ...
    if (cbzPPLDispatchAddr) {
        ...
        arm64_dec_cb_n_z(pfsec_read32(gXPF.kernelTextSection, cbzPPLDispatchAddr),
                         cbzPPLDispatchAddr,
                         NULL,
                         NULL,
        ...
        arm64_gen_b_c_cond(OPT_BOOL(false),
                           OPT_UINT64_NONE,
                           OPT_UINT64_NONE,
                           ARM64_COND_ANY,
                           &bCondAnyInst,
                           &bCondAnyMask);
        uint64_t bcondPPLDispatchAddr = pfsec_find_next_inst(gXPF.kernelTextSection,
                                                             ppl_enter,
                                                             30,
                                                             bCondAnyInst,
                                                             bCondAnyMask);
        ...
        uint64_t ppl_bootstrap_dispatch = 0;
        arm64_dec_b_c_cond(pfsec_read32(gXPF.kernelTextSection, bcondPPLDispatchAddr),
                           bcondPPLDispatchAddr,
                           &ppl_bootstrap_dispatch,
                           NULL,
                           NULL);
        return ppl_bootstrap_dispatch;
...
static uint64_t xpf_find_pmap_image4_trust_caches(void) {
    ...
    uint64_t ldrLitAddr = pfsec_find_next_inst(gXPF.kernelPPLTextSection,
...
static uint64_t xpf_find_pmap_query_trust_cache_safe(void) {
    ...
    uint64_t blAddr = pfsec_find_next_inst(gXPF.kernelPPLTextSection,
                                           pmap_lookup_in_loaded_trust_caches_internal,
                                           30,
                                           blAny,
                                           blAnyMask);
...
static uint64_t xpf_find_ppl_trust_cache_rt(void) {
    ...
    uint64_t pacMovAddr = pfsec_find_next_inst(gXPF.kernelPPLTextSection,
    ...
    uint64_t ppl_trust_cache_rt_refAddr = pfsec_find_prev_inst(gXPF.kernelPPLTextSection,
                                                               pacMovAddr,
                                                               10,
                                                               addX0AnyInst,
                                                               addX0AnyMask);
...
static uint64_t xpf_find_load_static_trust_cache(void) {
    PFStringMetric *trustcacheSegmentIsZeroMetric = pfmetric_string_init(
    ...
    PFXrefMetric *trustcacheSegmentIsZeroXrefMetric = pfmetric_xref_init(trustcacheSegmentIsZeroAddr,
...
static uint64_t xpf_find_phys_attribute_clear_with_flush_range(void) {
    ...
    PFStringMetric *physAttributeMetric = pfmetric_string_init(
    ...
    PFXrefMetric *physAttributeXrefMetric = pfmetric_xref_init(phys_attribute_clear_stringAddr,
                                                               XREF_TYPE_MASK_REFERENCE);
...
static uint64_t xpf_find_phys_attribute_clear_with_flush_range_reference(uint32_t idx) {
    uint64_t phys_attribute_clear_with_flush_range = xpf_item_resolve(
        "kernelSymbol.phys_attribute_clear_with_flush_range");
...
static uint64_t xpf_sptm_find_pmap_enter_options_addr(void) {
    ...
    PFStringMetric *stringMetric = pfmetric_string_init(
        "pmap_enter_options(): attempt to add executable mapping to kernel_pmap @%s:%d");
    XPF_ASSERT(stringMetric);
...
void xpf_ppl_init(void) {
    if (gXPF.kernelIsArm64e) {
        if (!gXPF.isSPTMDevice) {
            ...
            xpf_item_register("kernelSymbol.pmap_lookup_in_loaded_trust_caches_internal",
                              xpf_find_ppl_routine,
                              (void *)(uint32_t)41);
        ...
        if (gXPF.isSPTMDevice) {
            ...
            xpf_item_register("kernelSymbol.phys_attribute_clear_with_flush_range",
                              xpf_find_phys_attribute_clear_with_flush_range,
                              NULL);
            xpf_item_register("kernelSymbol.vm_first_phys",
                              xpf_find_phys_attribute_clear_with_flush_range_reference,
                              (void *)(uint32_t)0);
            xpf_item_register("kernelSymbol.vm_last_phys",
                              xpf_find_phys_attribute_clear_with_flush_range_reference,
                              (void *)(uint32_t)1);
            xpf_item_register("kernelSymbol.pp_attr_table",
                              xpf_find_phys_attribute_clear_with_flush_range_reference,
                              (void *)(uint32_t)2);
            ...
            xpf_item_register("kernelSymbol.vm_first_phys",
                              xpf_find_pmap_pin_kernel_pages_reference,
                              (void *)(uint32_t)0);
            xpf_item_register("kernelSymbol.vm_last_phys",
                              xpf_find_pmap_pin_kernel_pages_reference,
                              (void *)(uint32_t)1);
        ...
        if (strcmp(gXPF.darwinVersion, "22.0.0") >= 0) {
            ...
            if (gXPF.isSPTMDevice) {
                ...
                xpf_item_register("kernelSymbol.ppl_trust_cache_rt", xpf_find_sptm_trust_cache_rt, NULL);
            } else {
                xpf_item_register("kernelSymbol.pmap_query_trust_cache_safe",
                                  xpf_find_pmap_query_trust_cache_safe,
                                  NULL);
