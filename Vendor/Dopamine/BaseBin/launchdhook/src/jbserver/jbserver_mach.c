...
int jbserver_send_mach_reply(mach_msg_header_t *hdr, void *replyData) {
    ...
    if (replyData && MACH_PORT_VALID(hdr->msgh_remote_port) && MACH_MSGH_BITS_REMOTE(hdr->msgh_bits) != 0) {
        ...
        if (kr
            == KERN_SUCCESS /*|| kr == MACH_SEND_INVALID_MEMORY || kr == MACH_SEND_INVALID_RIGHT || kr == MACH_SEND_INVALID_TYPE || kr == MACH_SEND_MSG_TOO_SMALL*/) {
...
int jbserver_received_mach_message(audit_token_t *auditToken, struct jbserver_mach_msg *jbsMachMsg) {
    ...
    if (!roothide_domain_allowed(*auditToken))
        return -1;
    ...
    if (jbsMachMsg->action == JBSERVER_MACH_CHECKIN) {
        ...
        int result = systemwide_process_checkin(auditToken,
                                                &jbRootPath,
                                                &bootUUID,
                                                &sandboxExtensions,
                                                &reply->fullyDebugged);
        ...
        reply->base.status = result;
        r = 0;
    } else if (jbsMachMsg->action == JBSERVER_MACH_FORK_FIX_PREPARE
               || jbsMachMsg->action == JBSERVER_MACH_FORK_FIX_RESTORE) {
        if (msgSize < sizeof(struct jbserver_mach_msg_forkfix_trust)) {
            return -1;
        }
        ...
        if (!replyData)
            return -1;
        struct jbserver_mach_msg_forkfix_reply *reply = replyData;

        int result = jbsMachMsg->action == JBSERVER_MACH_FORK_FIX_PREPARE ? systemwide_fork_fix_prepare(auditToken)
                                                                          : systemwide_fork_fix_restore(auditToken);
        reply->base.msg.magic = jbsMachMsg->magic;
        reply->base.msg.action = jbsMachMsg->action;
        reply->base.msg.hdr.msgh_size = replySize;
        reply->base.status = result;
        ...
        int result = systemwide_trust_file(auditToken,
                                           trustMsg->fd,
