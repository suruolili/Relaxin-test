...
static void display_clear_state(void) {
    if (gDisplay.surface) {
...
int display_update(void) {
    ...
    IOMobileFramebufferSwapSetLayer(gDisplay.display,
                                    0,
                                    gDisplay.surface,
...
static int display_init_internal(bool useDCPFlags) {
    int r = find_target_display(&gDisplay.display);
    if (r)
        goto fail;
    gDisplay.size = find_display_size();
    if (gDisplay.size.width == 0 || gDisplay.size.height == 0) {
        r = -1;
        goto fail;
    }

    gDisplay.surface = create_iosurface_for_display(gDisplay.size,
                                                    useDCPFlags ? kIOMapWriteCombineCache | kIOMapInhibitCache
                                                            | kIOMapWriteThruCache | kIOMapCopybackCache
    ...
    if (IOSurfaceLock(gDisplay.surface, 0, 0) != KERN_SUCCESS) {
        r = -1;
        goto fail;
    }
    ...
    if (!gDisplay.base || gDisplay.bytesPerRow <= 0) {
        r = -1;
        goto fail;
    }
    ...
    if (kr != KERN_SUCCESS) {
        r = kr == kIOReturnBadMedia ? kIOReturnBadMedia : -1;
        goto fail;
    }
    return 0;
...
int display_init(void) {
    // A userspace reboot reinitializes the framebuffer while launchd survives.
    // Never reuse the IOSurface mapping retained by an earlier boot logo draw.
    display_clear_state();
...
int draw_image_to_buf(CGImageRef cgImage,
                      IOMobileFramebufferDisplaySize size,
                      CGFloat rotation,
                      void **bufOut,
                      size_t *bufSizeOut) {
    ...
    context = CGBitmapContextCreate(tmpBuf,
                                    size.width,
                                    size.height,
                                    8,
                                    bytesPerRow,
...
CGFloat get_main_screen_rotation(void) {
    if (is_ipad()) {
        ...
        if (mainScreenOrientationNum) {
            unsigned long long mainScreenOrientation = [(
                __bridge NSNumber *)mainScreenOrientationNum unsignedLongLongValue];
            if (mainScreenOrientation == 0) { // iPads that have a non landscape base orientation...
                ...
                unsigned long long displayBootRotation = [(__bridge NSNumber *)displayBootRotationNum
                    unsignedLongLongValue]; // ...need to take the displayBootRotation as the image rotation
                ...
                CFNumberRef displayBootRotationNum = MGCopyAnswer(CFSTR("DisplayBootRotation"));
                unsigned long long displayBootRotation = [(
                    __bridge NSNumber *)displayBootRotationNum unsignedLongLongValue];
                switch (displayBootRotation) {
                    case 0:
                        return 90;
                    case 90:
...
CGImageRef load_image_from_bitmap_plist(const char *bitmapPlistPath) {
    NSDictionary *imagePlist = [NSDictionary
        dictionaryWithContentsOfURL:[NSURL fileURLWithPath:[NSString stringWithUTF8String:bitmapPlistPath]]
                              error:nil];
    if (!imagePlist)
        return NULL;
...
int draw_image_path_to_buf(const char *image_path,
                           IOMobileFramebufferDisplaySize size,
                           CGFloat rotation,
                           void **bufOut,
                           size_t *bufSizeOut) {
