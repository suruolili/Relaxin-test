...
uint64_t getUserReturnThreadContext(void) {
    ...
    thread_t chThread = 0;
    kern_return_t kr = thread_create_running(mach_task_self_,
                                             ARM_THREAD_STATE64,
                                             (thread_state_t)&state,
                                             ARM_THREAD_STATE64_COUNT,
                                             &chThread);
    guard(kr == KERN_SUCCESS) else {
        JBLogError("getUserReturnThreadContext failed phase=create-return-thread");
        return 0;
    }
    ...
    guard(returnThreadPtr != 0) else {
        JBLogError("getUserReturnThreadContext failed phase=find-return-thread");
        return 0;
    }
    ...
    guard(returnThreadACTContext != 0) else {
        JBLogError("getUserReturnThreadContext failed phase=resolve-act-context");
        return 0;
    }
...
int fugu14_kcall_init(int (^threadSigner)(mach_port_t threadPort)) {
    ...
    guard(kr == KERN_SUCCESS) else {
        JBLogError("fugu14 kcall initialization failed phase=create-thread");
        return -1;
    }
    ...
    guard(threadPtr != 0) else {
        JBLogError("fugu14 kcall initialization failed phase=find-thread");
        return -1;
    }
    ...
    guard(threadPtr != 0) else {
        JBLogError("fugu14 kcall initialization failed phase=resolve-act-context");
        return -1;
    }
    ...
    if (kalloc_with_options(&stack, 0x4000 * 4, KALLOC_OPTION_LOCAL) != 0) { // Four pages
        JBLogError("fugu14 kcall initialization failed phase=allocate-stack");
        return -1;
    }
    stack += 0x8000;
    guard(stack != 0) else {
        JBLogError("fugu14 kcall initialization failed phase=allocate-kernel-stack");
        return -1;
    }
    ...
    if (threadSigner(thread) != 0) {
        JBLogError("fugu14 kcall initialization failed phase=sign-thread");
        return -1;
    }
