...
void dsc_enumerate_mappings(DyldSharedCache *sharedCache,
                            void (^enumeratorBlock)(DyldSharedCacheMapping *mapping,
                                                    DyldSharedCacheFile *sourceFile,
                                                    bool *stop)) {
...
DyldSharedCacheMapping *dsc_lookup_mapping(DyldSharedCache *sharedCache, uint64_t vmaddr, uint64_t size) {
    ...
    dsc_enumerate_mappings(sharedCache,
                           ^(DyldSharedCacheMapping *mapping, DyldSharedCacheFile *sourceFile, bool *stop) {
...
int dsc_read_from_vmaddr(DyldSharedCache *sharedCache, uint64_t vmaddr, size_t size, void *outBuf) {
    ...
    while (curAddr < endAddr) {
        ...
        if (copySize > mappingRemaining)
            copySize = mappingRemaining;

        memcpy((void *)((intptr_t)outBuf + (curAddr - startAddr)),
               (void *)((intptr_t)mapping->ptr + startOffset),
               copySize);
        curAddr += copySize;
    }
...
int dsc_read_string_from_vmaddr(DyldSharedCache *sharedCache, uint64_t vmaddr, char **outString) {
    ...
    char c = 0;
    do {
        if (dsc_read_from_vmaddr(sharedCache, vmaddr + (len++), sizeof(c), &c) != 0)
            return -1;
    } while (c != 0);
...
DyldSharedCacheFile *_dsc_load_file(const char *dscPath, const char suffix[32]) {
    ...
    if (strncmp(file->header.magic, "dyld_v", 6) != 0)
        goto fail;
    ...
    if (file->header.mappingOffset < sizeof(file->header)) {
        memset((void *)((uintptr_t)&file->header + file->header.mappingOffset),
               0,
               sizeof(file->header) - file->header.mappingOffset);
...
DyldSharedCache *dsc_init_from_path_premapped(const char *path, uint32_t premapSlide) {
    ...
    if (!strncmp(mainHeader->magic, "dyld_v1  armv7", 14)) {
        ...
        fprintf(stderr, "Error: DSC has unsupported architecture\n");
        return NULL;
    }
    ...
    if (subCacheArrayCount > 0) {
        ...
        int subCacheStructVersion = mainHeader->mappingOffset <= offsetof(struct dyld_cache_header, cacheSubType) ? 1
                                                                                                                  : 2;
        ...
        for (uint32_t i = 0; i < subCacheArrayCount; i++) {
            ...
            if (subCacheStructVersion == 1) {
                ...
                dsc_file_read_at_offset(mainFile,
                                        mainHeader->subCacheArrayOffset + sizeof(v1Entry) * i,
                                        sizeof(v1Entry),
                                        &v1Entry);
                ...
                dsc_file_read_at_offset(mainFile,
                                        mainHeader->subCacheArrayOffset + sizeof(subcacheEntry) * i,
                                        sizeof(subcacheEntry),
                                        &subcacheEntry);
            }
    ...
    for (unsigned i = 0; i < sharedCache->fileCount; i++) {
        ...
        sharedCache->mappings = realloc(sharedCache->mappings,
                                        sharedCache->mappingCount * sizeof(DyldSharedCacheMapping));
        ...
        for (int k = 0; k < header->mappingCount; k++) {
            ...
            if (slideInfoExists) {
                ...
                dsc_file_read_at_offset(file,
                                        mappingOffset + k * sizeof(mappingInfo),
                                        sizeof(mappingInfo),
                                        &mappingInfo);
            ...
            if (fullInfo.slideInfoFileOffset) {
                ...
                dsc_file_read_at_offset(file,
                                        fullInfo.slideInfoFileOffset,
                                        thisMapping->slideInfoSize,
                                        thisMapping->slideInfoPtr);
            } else {
    ...
    if (!sharedCache->symbolFile.loaded) {
        if (sharedCache->symbolFile.index != -1) {
            ...
            if (symbolCacheHeader->localSymbolsOffset) {
                ...
                dsc_file_read_at_offset(symbolCacheFile,
                                        symbolCacheHeader->localSymbolsOffset,
                                        sizeof(symbolsInfo),
                                        &symbolsInfo);
...
#undef _GENERIC_READ_SYMBOL_ENTRY
                    ...
                    DyldSharedCacheImage *image = dsc_lookup_image_by_address(sharedCache,
                                                                              sharedCache->baseAddress + dylibOffset);
                    if (image) {
                ...
                uint64_t nlistSize = (dsc_is32bit(sharedCache) ? sizeof(struct nlist) : sizeof(struct nlist_64))
                    * sharedCache->symbolFile.nlistCount;
                sharedCache->symbolFile.nlist = malloc(nlistSize);
                dsc_file_read_at_offset(symbolCacheFile,
                                        symbolCacheHeader->localSymbolsOffset + symbolsInfo.nlistOffset,
                                        nlistSize,
                                        sharedCache->symbolFile.nlist);

                uint64_t stringsOffsetPage = (symbolCacheHeader->localSymbolsOffset + symbolsInfo.stringsOffset)
                    & ~PAGE_MASK;
                uint64_t stringsOffsetPageOff = (symbolCacheHeader->localSymbolsOffset + symbolsInfo.stringsOffset)
                    & PAGE_MASK;

                sharedCache->symbolFile.stringsSize = symbolsInfo.stringsSize;

                char *mappedStrings = mmap(NULL,
                                           sharedCache->symbolFile.stringsSize + stringsOffsetPageOff,
                                           PROT_READ,
...
void dsc_enumerate_files(DyldSharedCache *sharedCache,
                         void (^enumeratorBlock)(const char *filepath,
                                                 size_t filesize,
                                                 struct dyld_cache_header *header)) {
    for (int i = 0; i < sharedCache->fileCount; i++) {
        enumeratorBlock(sharedCache->files[i]->filepath,
                        sharedCache->files[i]->filesize,
                        &sharedCache->files[i]->header);
    }
}

void dsc_enumerate_images(
    DyldSharedCache *sharedCache,
    void (^enumeratorBlock)(const char *path, DyldSharedCacheImage *imageHandle, MachO *imageMachO, bool *stop)) {
...
DyldSharedCacheImage *dsc_find_image_for_section_address(DyldSharedCache *sharedCache, uint64_t address) {
    ...
    for (unsigned i = 0; i < sharedCache->containedImageCount; i++) {
        if (sharedCache->containedImages[i].fat->slicesCount == 1) {
            ...
            if (macho) {
                macho_enumerate_sections(macho,
                                         ^(struct section_64 *section, struct segment_command_64 *segment, bool *stop) {
                                             if (address >= section->addr
...
MachO *dsc_lookup_macho_by_address(DyldSharedCache *sharedCache,
                                   uint64_t address,
                                   DyldSharedCacheImage **imageHandleOut) {
    DyldSharedCacheImage *image = dsc_lookup_image_by_address(sharedCache, address);
    if (image) {
        if (imageHandleOut)
            *imageHandleOut = image;
...
int dsc_image_enumerate_symbols(DyldSharedCache *sharedCache,
                                DyldSharedCacheImage *image,
                                void (^enumeratorBlock)(const char *name, uint8_t type, uint64_t vmaddr, bool *stop)) {
    if (_dsc_load_symbols(sharedCache) != 0)
        return -1;
    ...
    if (lastSymIdx > sharedCache->symbolFile.nlistCount)
        return -1;
...
#undef _GENERIC_READ_NLIST
        ...
        if (n_strx > sharedCache->symbolFile.stringsSize)
            return -1;
...
int dsc_image_enumerate_patches(DyldSharedCache *sharedCache,
                                DyldSharedCacheImage *image,
                                void (^enumeratorBlock)(unsigned v, void *patchable_location, bool *stop)) {
    struct dyld_cache_header *mainHeader = &sharedCache->files[0]->header;

    struct dyld_cache_patch_info_v3 *patchInfo = dsc_find_buffer(sharedCache,
                                                                 mainHeader->patchInfoAddr,
                                                                 mainHeader->patchInfoSize);

    if (patchInfo->infoV2.patchTableVersion == 4) {
        struct dyld_cache_image_got_clients_v3 *gotClients = (void *)((uintptr_t)patchInfo
                                                                      + (patchInfo->gotClientsArrayAddr
                                                                         - mainHeader->patchInfoAddr));
        struct dyld_cache_patchable_export_v3 *clientExports = (void *)((uintptr_t)patchInfo
                                                                        + (patchInfo->gotClientExportsArrayAddr
                                                                           - mainHeader->patchInfoAddr));
        struct dyld_cache_patchable_location_v4 *locationArray = (void *)((uintptr_t)patchInfo
...
int dsc_mapping_enumerate_chained_fixups_on_range(DyldSharedCache *sharedCache,
                                                  DyldSharedCacheMapping *mapping,
                                                  uint64_t rangeStart,
                                                  uint64_t rangeEnd,
                                                  void (^enumeratorBlock)(DyldSharedCachePointer *pointer,
                                                                          bool *stop)) {
    if (!mapping->slideInfoPtr)
        return -1;
    if (sharedCache->premapSlide != 0)
        return -1; // XXX: Not supported on premapped dscs
    ...
    switch (version) {
        case 5: {
            ...
            if (rangeStart != 0 && rangeEnd != 0) {
                if (rangeStart < startAddr || rangeStart >= endAddr)
                    return -1;
                startAddr = rangeStart & ~pageMask;
                if (rangeEnd <= startAddr || rangeEnd > endAddr)
                    return -1;
            ...
            for (uint64_t pageAddr = startAddr; pageAddr < endAddr; pageAddr += info->page_size) {
                ...
                if (delta != DYLD_CACHE_SLIDE_V5_PAGE_ATTR_NO_REBASE) {
                    ...
                    do {
                        ...
                        if (hasRange) {
                            if (pointer.location < rangeStart)
                                continue;
                            if (pointer.location >= rangeEnd)
                                break;
                        }
            ...
            for (uint64_t pageAddr = startAddr; pageAddr < endAddr; pageAddr += info->page_size) {
                ...
                int (^processChain)(uint32_t pageOffset) = ^(uint32_t pageOffset) {
                    ...
                    while (delta != 0) {
                        ...
                        uint64_t rawValue = 0;
                        dsc_read_from_vmaddr(sharedCache,
                                             locAddr,
                                             dsc_is32bit(sharedCache) ? sizeof(uint32_t) : sizeof(uint64_t),
                                             &rawValue);
...
int dsc_mapping_enumerate_chained_fixups(DyldSharedCache *sharedCache,
                                         DyldSharedCacheMapping *mapping,
                                         void (^enumeratorBlock)(DyldSharedCachePointer *pointer, bool *stop)) {
    return dsc_mapping_enumerate_chained_fixups_on_range(sharedCache, mapping, 0, 0, enumeratorBlock);
}

int dsc_enumerate_chained_fixups(DyldSharedCache *sharedCache,
                                 void (^enumeratorBlock)(DyldSharedCachePointer *pointer, bool *stop)) {
...
int dsc_image_enumerate_chained_fixups(DyldSharedCache *sharedCache,
                                       DyldSharedCacheImage *image,
                                       void (^enumeratorBlock)(DyldSharedCachePointer *pointer, bool *stop)) {
    /*
    ...
    if (macho) {
        ...
        macho_enumerate_segments(macho, ^(struct segment_command_64 *segment, bool *stop) {
            ...
            if (!strncmp(segment->segname, "__DATA", 6)) /*__DATA, __DATA_DIRTY, __DATA_CONST */ {
                ...
                if (mapping) {
                    dsc_mapping_enumerate_chained_fixups_on_range(sharedCache,
                                                                  mapping,
                                                                  segment->vmaddr,
                                                                  segment->vmaddr + segment->vmsize,
                                                                  ^(DyldSharedCachePointer *pointer, bool *stop2) {
                                                                      enumeratorBlock(pointer, &stopAll);
                                                                      *stop2 = stopAll;
                                                                  });
                }
