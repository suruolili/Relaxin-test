...
int macho_parse_signature_blob_to_der_encoded_data(MachO *macho,
                                                   uint32_t signatureBlobOffset,
...
int macho_find_code_signature_bounds(MachO *macho, uint32_t *offsetOut, uint32_t *sizeOut) {
    ...
    macho_enumerate_load_commands(macho, ^(struct load_command loadCommand, uint64_t offset, void *cmd, bool *stop) {
        if (loadCommand.cmd == LC_CODE_SIGNATURE) {
            ...
            if (sizeOut)
                *sizeOut = csLoadCommand->datasize;
            *stop = true;
            r = 0;
        }
    });
...
int macho_replace_code_signature(MachO *macho, CS_SuperBlob *superblob) {
    ...
    memory_stream_read(macho->stream,
                       csSegmentOffset + offsetof(CS_SuperBlob, length),
                       sizeof(sizeOfCodeSignature),
                       &sizeOfCodeSignature);
...
CS_DecodedBlob *csd_blob_init(uint32_t type, CS_GenericBlob *blobData) {
    ...
    blob->type = type;
    blob->stream = buffered_stream_init_from_buffer(blobData,
                                                    BIG_TO_HOST(blobData->length),
...
int csd_superblob_insert_blob_after_blob(CS_DecodedSuperBlob *superblob,
                                         CS_DecodedBlob *blobToInsert,
                                         CS_DecodedBlob *afterBlob) {
...
CS_DecodedBlob *csd_superblob_find_best_code_directory(CS_DecodedSuperBlob *decodedSuperblob) {
    ...
    while (blob) {
        if (blob->type == CSSLOT_CODEDIRECTORY
            || ((CSSLOT_ALTERNATE_CODEDIRECTORIES <= blob->type
                 && blob->type < CSSLOT_ALTERNATE_CODEDIRECTORY_LIMIT))) {
...
int csd_superblob_calculate_best_cdhash(CS_DecodedSuperBlob *decodedSuperblob, void *cdhashOut, int *cdhashTypeOut) {
    ...
    if (cdhashTypeOut)
        *cdhashTypeOut = csd_code_directory_get_hash_type(bestCDBlob);
    return csd_code_directory_calculate_hash(bestCDBlob, cdhashOut);
}

int csd_superblob_print_content(CS_DecodedSuperBlob *decodedSuperblob,
                                MachO *macho,
                                bool printAllSlots,
                                bool verifySlots) {
