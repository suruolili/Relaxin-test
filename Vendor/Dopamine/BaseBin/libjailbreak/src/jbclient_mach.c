...
int jbclient_mach_process_checkin(char *jbRootPathOut,
                                  char *bootUUIDOut,
    ...
    if (jbRootPathOut)
        strcpy(jbRootPathOut, reply->jbRootPath);

    reply->bootUUID[sizeof(reply->bootUUID) - 1] = '\0';
    if (bootUUIDOut)
        strcpy(bootUUIDOut, reply->bootUUID);

    reply->sandboxExtensions[sizeof(reply->sandboxExtensions) - 1] = '\0';
    if (sandboxExtensionsOut)
        strcpy(sandboxExtensionsOut, reply->sandboxExtensions);

    if (fullyDebuggedOut)
        *fullyDebuggedOut = reply->fullyDebugged;
...
static int jbclient_mach_fork_fix_trust(uint64_t action) {
    struct jbserver_mach_msg_forkfix_trust msg = {0};
    msg.base.hdr.msgh_size = sizeof(msg);
    msg.base.action = action;
    msg.base.magic = JBSERVER_MACH_MAGIC;
...
int jbclient_mach_fork_fix_prepare(void) {
    return jbclient_mach_fork_fix_trust(JBSERVER_MACH_FORK_FIX_PREPARE);
}

int jbclient_mach_fork_fix_restore(void) {
    return jbclient_mach_fork_fix_trust(JBSERVER_MACH_FORK_FIX_RESTORE);
}
