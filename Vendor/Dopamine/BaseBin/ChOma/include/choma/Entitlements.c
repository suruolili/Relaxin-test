...
CFDataRef get_property_list_data(CFPropertyListRef plist) {
    ...
    data = CFPropertyListCreateData(kCFAllocatorDefault, plist, kCFPropertyListXMLFormat_v1_0, 0, &error);
...
CFPropertyListRef create_property_list_from_file(const char *xmlFile) {
    ...
    CFErrorRef error;
    CFPropertyListRef entitlementsPlist = CFPropertyListCreateWithData(NULL,
                                                                       entitlementsData,
...
CS_DecodedBlob *create_xml_entitlements_blob(const char *entitlementsFile) {
    ...
    uint32_t *genericBlob = malloc(sizeof(CSMAGIC_EMBEDDED_ENTITLEMENTS) + sizeof(uint32_t)
                                   + CFDataGetLength(entitlementsPlistData));
    if (genericBlob == NULL) {
    ...
    *genericBlob = HOST_TO_BIG(CSMAGIC_EMBEDDED_ENTITLEMENTS);
    uint32_t totalSize = sizeof(CSMAGIC_EMBEDDED_DER_ENTITLEMENTS) + sizeof(uint32_t)
        + CFDataGetLength(entitlementsPlistData);
    *(genericBlob + 1) = HOST_TO_BIG(totalSize);
    memcpy((char *)genericBlob + sizeof(CSMAGIC_EMBEDDED_ENTITLEMENTS) + sizeof(uint32_t),
           CFDataGetBytePtr(entitlementsPlistData),
           CFDataGetLength(entitlementsPlistData));
...
CS_DecodedBlob *create_der_entitlements_blob(const char *entitlementsFile) {
    ...
    *(genericBlob + 1) = HOST_TO_BIG(totalSize);
    ...
    memcpy((char *)genericBlob + sizeof(CSMAGIC_EMBEDDED_DER_ENTITLEMENTS) + sizeof(uint32_t),
           entitlements->data,
           entitlements->length);
