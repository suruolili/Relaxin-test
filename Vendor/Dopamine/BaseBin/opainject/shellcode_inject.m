...
static kern_return_t runPayload(task_t task,
                                uint8_t *payload,
                                size_t payloadSize,
                                uint64_t codeStart,
    ...
    vm_address_t libSystemPthreadAddr = getRemoteImageAddress(task,
                                                              allImageInfoAddr,
                                                              "/usr/lib/system/libsystem_pthread.dylib");
    uint64_t pthread_create_from_mach_threadAddr = remoteDlSym(task,
                                                               libSystemPthreadAddr,
    ...
    size_t bootstrapPayloadSize = bootstrapCodeVarCount * sizeof(uint32_t)
        + sizeof(bootstrapCode); // allocate needed memory space
    ...
    memcpy((void *)(bootstrapPayloadPtr + bootstrapCodeVarSize),
           &bootstrapCode[0],
           sizeof(bootstrapCode)); //insert code
    ...
    kr = vm_protect(task,
                    remoteBootstrapPayload + bootstrapCodeVarSize,
    ...
    memcpy((void *)(payloadSuffixPtr + payloadSuffixCodeSize),
           (const void *)&pthread_exitAddr,
           sizeof(uint64_t)); // insert variables
    ...
    kr = vm_protect(task,
                    remotePayload + codeStart,
                    fullPayloadSize - codeStart - payloadSuffixVarSize,
                    FALSE,
                    VM_PROT_READ | VM_PROT_EXECUTE);
    if (kr != KERN_SUCCESS) {
        free(payloadSuffix);
    ...
    printf("marked %llX - %llX as rx\n",
           (uint64_t)remotePayload + codeStart,
           (uint64_t)remotePayload + (fullPayloadSize - codeStart - payloadSuffixVarSize));
    ...
    __darwin_arm_thread_state64_set_pc_fptr(remoteThreadState64.ts_64,
                                            (void *)(remoteBootstrapPayload + bootstrapCodeVarSize));
    remoteThreadState64.ts_64.__x[2] = remotePayload
        + codeStart; // <- startFunc argument of pthread_create_from_mach_thread call
    ...
    for (;;) {
        kr = thread_get_state(remoteThread,
                              ARM_THREAD_STATE64,
                              (thread_state_t)&remoteThreadState64.ts_64,
                              &thread_state_count);
        if (kr != KERN_SUCCESS) {
...
int injectDylibViaShellcode(task_t task, pid_t pid, const char *dylibPath, vm_address_t allImageInfoAddr) {
    ...
    vm_address_t libSystemSandboxAddr = getRemoteImageAddress(task,
                                                              allImageInfoAddr,
    ...
    // STEP TWO: apply sandbox extension if needed

    // Check whether sandbox extension is needed
    int sandboxExtensionNeeded = sandbox_check(pid,
                                               "file-read-data",
    ...
    if (sandboxExtensionNeeded) {
        ...
        memcpy((void *)(payloadIntPtr + stringAllocSize),
               (const void *)&sandbox_extension_consumeAddr,
               sizeof(uint64_t)); // insert vars
