...
int jbclient_jailbreakd_checkin_status(bool *checkedInOut) {
    ...
    *checkedInOut = false;
    ...
    xpc_object_t xrequest = xpc_dictionary_create_empty();
    xpc_dictionary_set_uint64(xrequest, "jb-domain", JBS_DOMAIN_ROOTHIDE);
    xpc_dictionary_set_uint64(xrequest, "action", JBS_ROOTHIDE_JAILBREAKD_CHECKIN_STATUS);
    xpc_object_t xreply = jbserver_xpc_send_dict_fresh(xrequest);
    xpc_release(xrequest);
    if (!xreply)
        return -1;

    int64_t result = xpc_dictionary_get_int64(xreply, "result");
    if (result == 0) {
        *checkedInOut = xpc_dictionary_get_bool(xreply, "checked-in");
    }
    xpc_release(xreply);
    return (int)result;
}
