...
int host_get_cpu_information(cpu_type_t *cputype, cpu_subtype_t *cpusubtype) {
    ...
    if (sysctlbyname("hw.cputype", cputype, &len, NULL, 0) == -1) {
        fprintf(stderr, "Error: no cputype.\n");
        return -1;
    }
    ...
    if (sysctlbyname("hw.cpusubtype", cpusubtype, &len, NULL, 0) == -1) {
        fprintf(stderr, "Error: no cpusubtype.\n");
        return -1;
    }
...
MachO *fat_find_preferred_slice(Fat *fat) {
    ...
    if (!preferredMacho) {
        fprintf(stderr, "Error: failed to find a preferred MachO slice that matches the host architecture.\n");
    }
