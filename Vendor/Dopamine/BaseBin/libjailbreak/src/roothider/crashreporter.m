...
__attribute__((noinline)) static void pthread_backtrace(pthread_t pthread,
                                                        vm_address_t *buffer,
                                                        unsigned max,
                                                        unsigned *nb,
                                                        unsigned skip,
...
void crashreporter_dump_backtrace_line(FILE *f, vm_address_t addr) {
    ...
    if (dladdr((void *)addr, &info) != 0) {
        ...
        fprintf(f,
                "0x%lX: %s (0x%lX + 0x%lX) (%s(0x%lX) + 0x%lX)\n",
                addr,
                sname,
                (vm_address_t)info.dli_saddr,
                addr - (vm_address_t)info.dli_saddr,
                fname,
                (vm_address_t)info.dli_fbase,
                addr - (vm_address_t)info.dli_fbase);
    } else {
...
void crashreporter_dump_mach(FILE *f,
                             int exception,
                             int ncode,
                             int64_t *code,
    ...
    threadState.__opaque_flags &= ~(__DARWIN_ARM_THREAD_STATE64_FLAGS_IB_SIGNED_LR
    ...
    fprintf(
        f,
        " lr = 0x%016llX,  pc = 0x%016llX,  sp = 0x%016llX,  fp = 0x%016llX, cpsr= 0x%08X,       flags = 0x%08X\nesr = 0x%08X,         far = 0x%016llX\n\n",
    ...
    fprintf(f,
            " pc = 0x%016llX,  lr = 0x%016llX,  sp = 0x%016llX,  fp = 0x%016llX\n\n",
...
void crashreporter_catch_mach(__Request__mach_exception_raise_t *request, __Reply__mach_exception_raise_t *reply) {
    ...
    thread_get_state(request->thread.name,
    ...
    ABORT("Mach exception occured on thread %llu. A detailed report has been written to the file %s.",
          tid,
          name ? name : "(null)");
}
...
void crashreporter_catch_objc(NSException *e) {
    @autoreleasepool {
        ...
        ABORT("Objective-C exception occured on thread %llu. A detailed report has been written to the file %s.",
              tid,
              name ? name : "(null)");
    }
}
...
void *crashreporter_listen(void *arg) {
    ...
    while (true) {
        ...
        if (pid != getpid()) {
            ABORT("Mach Exception(%d,%llx,%llx) from another process: %d",
                  request->exception,
                  request->code[0],
                  request->code[1],
                  pid);
        ...
        crashreporter_catch_mach(request, &reply);
    ...
    @synchronized(@"CrashReporterStateKey") {
        if (key == gCrashReporterStateKey) {
            if (gCrashReporterState == kCrashReporterStatePaused) {
                task_set_exception_ports(mach_task_self_,
                                         EXC_MASK_CRASH_RELATED,
                                         gExceptionPort,
            ...
            JBLogError("crashreporter_resume called with mismatched key: %d (current key: %d)",
                       key,
                       gCrashReporterStateKey);
        }
    }
}
...
void signal_handler(int signo, siginfo_t *info, void *context) {
    ...
    if (f) {
        ...
        fprintf(f,
                "Signal %s(%d/%d), errno=%d, code=%d, status=%d, addr=%p, value=%p, band=%lx\n\n",
                strsignal(signo),
                signo,
                info->si_signo,
                info->si_errno,
        ...
        if (symbols != NULL) {
            ...
            fprintf(f, "No backtrace captured.\n");
            return;
        }
    ...
    ABORT("Unexpected signal %d on thread %llu, A detailed report has been written to the file %s.",
          signo,
          tid,
          name ? name : "(null)");
}
...
void crashreporter_start() {
    if (@available(iOS 17.0, *)) {
        // Dopamine 3.x disables this reporter in launchd. RootHide also uses
        // this implementation in jailbreakd and boomerang, so keep their
        // existing signal-reporting behavior unchanged.
        if (getpid() == 1)
            return;
    }
