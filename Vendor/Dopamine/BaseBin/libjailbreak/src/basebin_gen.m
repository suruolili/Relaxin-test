...
int apply_dyld_patch(NSString *dyldPath, const char *newUUIDPrefix) {
    ...
    if (getAMFIAddr == 0) {
        JBLogError("basebin dyld patch failed phase=find-getAMFI");
        return -1;
    }
                                  ...
                                  ^(struct load_command loadCommand, uint64_t offset, void *cmd, bool *stop) {
                                      if (loadCommand.cmd == LC_UUID) {
                                          ...
                                          if (newUUIDPrefixLen <= sizeof(uuid_t)) {
                                              ...
                                              r = -1;
                                              JBLogError("basebin dyld patch failed phase=write-uuid length=%zu",
                                                         newUUIDPrefixLen);
                                          }
                                          *stop = true;
                                      }
...
NSString *dyldhook_dylib_for_platform(void) {
    ...
    if (sysctlbyname("hw.cpusubtype", &cpusubtype, &len, NULL, 0) == -1) {
        return nil;
    }
...
int merge_dyldhook(NSString *originalDyldPath, NSString *outPath) {
    ...
    if (!dyldhookMergeDylibName) {
        JBLogError("basebin generation failed phase=locate-dyldhook");
        return -1;
    }

    NSString *dyldhookMergeDylibPath = [JBROOT_PATH(@"/basebin") stringByAppendingPathComponent:dyldhookMergeDylibName];
    int r = exec_cmd(JBROOT_PATH("/basebin/MachOMerger"),
                     originalDyldPath.fileSystemRepresentation,
                     dyldhookMergeDylibPath.fileSystemRepresentation,
...
int basebin_generate(bool comingFromJBUpdate) {
    ...
    [[NSFileManager defaultManager] createDirectoryAtPath:genPath withIntermediateDirectories:YES attributes:nil
                                                    error:nil];
    ...
    NSString *dopamineVersion = [NSString stringWithContentsOfFile:JBROOT_PATH(@"/basebin/.version")
    ...
    if (!comingFromJBUpdate) {
        ...
        [[NSFileManager defaultManager] createDirectoryAtPath:fakelibPath withIntermediateDirectories:YES attributes:nil
                                                        error:nil];
        ...
        [[NSFileManager defaultManager] createSymbolicLinkAtPath:fakelibDyldPath withDestinationPath:dyldPatchedPath
                                                           error:nil];

        // Symlink .fakelib/systemhook.dylib -> /var/jb/basebin/systemhook.dylib
        [[NSFileManager defaultManager] createSymbolicLinkAtPath:fakelibSystemHookPath
                                             withDestinationPath:systemhookPath
                                                           error:nil];
