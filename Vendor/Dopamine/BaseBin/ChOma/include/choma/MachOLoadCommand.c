...
void update_segment_command_64(MachO *macho,
...
int update_load_commands_for_coretrust_bypass(MachO *macho,
                                              CS_SuperBlob *superblob,
                                              uint64_t originalCodeSignatureSize) {
    ...
    if (blockPaddingSize == 0 || vmAddress == 0 || fileOffset == 0) {
        fprintf(stderr, "Error: failed to get existing values for __LINKEDIT segment.\n");
        return -1;
    }
