...
int macho_read_uleb128_at_offset(MachO *macho,
                                 uint64_t offset,
                                 uint64_t maxOffset,
                                 uint64_t *endOffsetOut,
                                 uint64_t *valueOut) {
    uint64_t result = 0;
    int bit = 0;
...
int macho_enumerate_load_commands(
    MachO *macho,
    void (^enumeratorBlock)(struct load_command loadCommand, uint64_t offset, void *cmd, bool *stop)) {
    if (macho->machHeader.ncmds < 1 || macho->machHeader.ncmds > 1000) {
        fprintf(stderr, "Error: invalid number of load commands (%d).\n", macho->machHeader.ncmds);
        return -1;
    }
    ...
    for (int j = 0; j < macho->machHeader.ncmds; j++) {
        ...
        if (macho_read_at_offset(macho, offset, sizeof(loadCommand), &loadCommand) != 0)
            continue;
...
int macho_enumerate_sections(MachO *macho,
                             void (^enumeratorBlock)(struct section_64 *section,
...
int macho_read_trie_node_at_offset(MachO *macho,
                                   uint64_t offset,
                                   uint64_t maxOffset,
                                   uint64_t *endOffsetOut,
                                   struct trie_node **trieNodesOut,
                                   unsigned *trieNodesCountOut) {
    if (!trieNodesOut || !trieNodesCountOut)
        return -1;
...
int macho_enumerate_symbols(MachO *macho,
...
#undef _GENERIC_READ_NLIST
            ...
            if (n_strx >= symtabCommand.strsize || n_strx == 0)
                continue;
...
int macho_enumerate_dependencies(
    MachO *macho,
    void (^enumeratorBlock)(const char *dylibPath, uint32_t cmd, struct dylib *dylib, bool *stop)) {
    macho_enumerate_load_commands(macho, ^(struct load_command loadCommand, uint64_t offset, void *cmd, bool *stop) {
        if (loadCommand.cmd == LC_LOAD_DYLIB || loadCommand.cmd == LC_LOAD_WEAK_DYLIB
            || loadCommand.cmd == LC_REEXPORT_DYLIB || loadCommand.cmd == LC_LAZY_LOAD_DYLIB
            ...
            if (dylibCommand->dylib.name.offset >= loadCommand.cmdsize
                || dylibCommand->dylib.name.offset < sizeof(struct dylib_command)) {
                fprintf(stderr, "WARNING: Malformed dependency at 0x%llx (Name offset out of bounds)\n", offset);
                return;
            }
            ...
            if (!dependencyLength) {
                fprintf(stderr, "WARNING: Malformed dependency at 0x%llx (Name has zero length)\n", offset);
                return;
            }
            if (dependencyPath[dependencyLength] != 0) {
                fprintf(stderr, "WARNING: Malformed dependency at 0x%llx (Name has non NULL end byte)\n", offset);
                return;
            }
...
int macho_enumerate_rpaths(MachO *macho, void (^enumeratorBlock)(const char *rpath, bool *stop)) {
    macho_enumerate_load_commands(macho, ^(struct load_command loadCommand, uint64_t offset, void *cmd, bool *stop) {
        if (loadCommand.cmd == LC_RPATH) {
            ...
            if (rpathCommand->path.offset >= loadCommand.cmdsize
                || rpathCommand->path.offset < sizeof(struct rpath_command)) {
                fprintf(stderr, "WARNING: Malformed rpath at 0x%llx (Path offset out of bounds)\n", offset);
                return;
            }
            ...
            if (!rpathLength) {
                fprintf(stderr, "WARNING: Malformed rpath at 0x%llx (Path has zero length)\n", offset);
                return;
            }
            if (rpath[rpathLength] != 0) {
                fprintf(stderr, "WARNING: Malformed rpath at 0x%llx (Name has non NULL end byte)\n", offset);
                return;
            }
...
int macho_enumerate_function_starts(MachO *macho, void (^enumeratorBlock)(uint64_t funcAddr, bool *stop)) {
    ...
    if (!functionStartsDataOff || !functionStartsDataSize)
        return -1;
...
int macho_parse_segments(MachO *macho) {
    return macho_enumerate_load_commands(macho,
                                         ^(struct load_command loadCommand, uint64_t offset, void *cmd, bool *stop) {
                                             if (macho->is32Bit && loadCommand.cmd == LC_SEGMENT) {
                                                 ...
                                                 if (macho->segments == NULL) {
                                                     macho->segments = malloc(macho->segmentCount
                                                                              * sizeof(MachOSegment *));
                                                 } else {
                                                     macho->segments = realloc(macho->segments,
                                                                               macho->segmentCount
                                                                                   * sizeof(MachOSegment *));
                                                 }
                                                 ...
                                                 SEGMENT_COMMAND_APPLY_BYTE_ORDER(&segmentCommand,
                                                                                  LITTLE_TO_HOST_APPLIER);

                                                 MachOSegment **segment = &macho->segments[macho->segmentCount - 1];

                                                 *segment = malloc(sizeof(MachOSegment)
                                                                   + segmentCommand.nsects * sizeof(struct section_64));
                                                 ...
                                                 memcpy((*segment)->command.segname,
                                                        segmentCommand.segname,
                                                        sizeof(segmentCommand.segname));

                                                 for (uint32_t i = 0;
                                                      i < macho->segments[macho->segmentCount - 1]->command.nsects;
                                                      i++) {
                                                     ...
                                                     memcpy((*segment)->sections[i].sectname,
                                                            section.sectname,
                                                            sizeof(section.sectname));
                                                     memcpy((*segment)->sections[i].segname,
                                                            section.segname,
                                                 ...
                                                 memcpy(macho->segments[macho->segmentCount - 1],
                                                        cmd,
                                                        loadCommand.cmdsize);
                                                 SEGMENT_COMMAND_64_APPLY_BYTE_ORDER(&macho
                                                                                          ->segments[macho->segmentCount
                                                                                                     - 1]
                                                                                          ->command,
                                                                                     LITTLE_TO_HOST_APPLIER);
                                                 for (uint32_t i = 0;
                                                      i < macho->segments[macho->segmentCount - 1]->command.nsects;
                                                      i++) {
                                                     SECTION_64_APPLY_BYTE_ORDER(&macho
                                                                                      ->segments[macho->segmentCount
                                                                                                 - 1]
                                                                                      ->sections[i],
                                                                                 LITTLE_TO_HOST_APPLIER);
...
int macho_parse_fileset_machos(MachO *macho) {
    if (macho_get_filetype(macho) != MH_FILESET)
        return -1;
    return macho_enumerate_load_commands(macho,
                                         ^(struct load_command loadCommand, uint64_t offset, void *cmd, bool *stop) {
                                             if (loadCommand.cmd == LC_FILESET_ENTRY) {
                                                 ...
                                                 FILESET_ENTRY_COMMAND_APPLY_BYTE_ORDER(filesetCommand,
                                                                                        LITTLE_TO_HOST_APPLIER);

                                                 if (macho->filesetMachos == NULL) {
                                                     macho->filesetMachos = malloc(macho->filesetCount
                                                                                   * sizeof(FilesetMachO));
                                                 } else {
                                                     macho->filesetMachos = realloc(macho->filesetMachos,
                                                                                    macho->filesetCount
                                                                                        * sizeof(FilesetMachO));
                                                 }

                                                 FilesetMachO *filesetMacho = &macho->filesetMachos[i];
                                                 filesetMacho->entry_id = strdup((char *)cmd
                                                                                 + filesetCommand->entry_id.offset);
...
int _macho_parse(MachO *macho) {
    macho->is32Bit = (macho->archDescriptor.cpusubtype == CPU_SUBTYPE_ARM_V6
                      || macho->archDescriptor.cpusubtype == CPU_SUBTYPE_ARM_V7
                      || macho->archDescriptor.cpusubtype == CPU_SUBTYPE_ARM_V7S);
    if (macho->machHeader.sizeofcmds % (macho->is32Bit ? sizeof(uint32_t) : sizeof(uint64_t)) != 0) {
        fprintf(stderr, "Error: sizeofcmds is not a multiple of %lu (%d).\n",
               macho->is32Bit ? sizeof(uint32_t) : sizeof(uint64_t),
...
MachO *macho_init(MemoryStream *stream, struct fat_arch_64 archDescriptor) {
    ...
    if (macho_read_at_offset(macho, 0, sizeof(macho->machHeader), &macho->machHeader) != 0)
        goto fail;
...
MachO *macho_init_for_writing(const char *filePath) {
    ...
    macho->stream = file_stream_init_from_path(filePath,
                                               0,
                                               FILE_STREAM_SIZE_AUTO,
...
MachO **macho_array_create_for_paths(char **inputPaths, int inputPathsCount) {
    ...
    for (int i = 0; i < inputPathsCount; i++) {
        ...
        if (!fat) {
            fprintf(stderr, "Error: failed to create Fat from file: %s\n", inputPaths[i]);
            return NULL;
        }
