...
extern int posix_spawnattr_set_persona_groups_np(const posix_spawnattr_t *__restrict, int, gid_t *__restrict, uid_t);
...
#define exec_cmd_trusted(x, args...) ({ \
    int retval; \
    int trustStatus = jbclient_trust_file_by_path(x); \
    if(trustStatus == 0) { \
		retval = exec_cmd(x, args); \
	} else { \
		JBLogError("exec_cmd_trusted: trust failed binary=%s status=%d; spawn blocked", x, trustStatus); \
		retval = -1; \
	} \
    retval; \
})
