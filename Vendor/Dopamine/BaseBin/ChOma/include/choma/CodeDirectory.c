...
bool code_directory_calculate_page_hash(CS_CodeDirectory *codeDir, MachO *macho, int slot, uint8_t *pageHashOut) {
    if (!codeDir || !macho || !pageHashOut || slot < 0 || (uint32_t)slot >= codeDir->nCodeSlots)
        return false;
    if (codeDir->pageSize >= 32)
        return false;

    uint64_t pageSize = UINT64_C(1) << codeDir->pageSize;
    if ((uint64_t)(uint32_t)slot > UINT64_MAX / pageSize)
        return false;
    uint64_t pageOffset = (uint64_t)(uint32_t)slot * pageSize;

    size_t streamSize = memory_stream_get_size(macho_get_stream(macho));
    if (streamSize == MEMORY_STREAM_SIZE_INVALID || pageOffset > streamSize)
        return false;

    // Special case for reading the code signature itself
    if (slot == codeDir->nCodeSlots - 1) {
        uint32_t csOffset = 0, csSize = 0;
        bool hasCodeSignature = macho_find_code_signature_bounds(macho, &csOffset, &csSize) == 0 && csOffset && csSize;
        if (hasCodeSignature) {
            if (pageOffset > csOffset)
                return false;
            pageSize = csOffset - pageOffset;
        } else {
            pageSize = streamSize - pageOffset;
        }
    }

    // Bail out when past EOF
    if (pageSize == 0 || pageSize > UINT32_MAX || pageSize > streamSize - pageOffset)
        return false;

    uint8_t *page = malloc((size_t)pageSize);
    if (!page)
        return false;
    if (macho_read_at_offset(macho, pageOffset, (size_t)pageSize, page) != 0) {
        free(page);
        return false;
    }
    ...
    switch (codeDir->hashType) {
        case CS_HASHTYPE_SHA160_160: {
            if (codeDir->hashSize != CC_SHA1_DIGEST_LENGTH) {
                success = false;
                break;
            }
            CC_SHA1(page, (CC_LONG)pageSize, pageHashOut);
            break;
        }

        case CS_HASHTYPE_SHA256_256:
        case CS_HASHTYPE_SHA256_160: {
            size_t expectedHashSize = codeDir->hashType == CS_HASHTYPE_SHA256_256 ? CC_SHA256_DIGEST_LENGTH
                                                                                  : CS_CDHASH_LEN;
            if (codeDir->hashSize != expectedHashSize) {
                success = false;
                break;
            }
            uint8_t fullHash[CC_SHA256_DIGEST_LENGTH];
            CC_SHA256(page, (CC_LONG)pageSize, fullHash);
            memcpy(pageHashOut, fullHash, codeDir->hashSize);
            break;
        }

        case CS_HASHTYPE_SHA384_384: {
            if (codeDir->hashSize != CC_SHA384_DIGEST_LENGTH) {
                success = false;
                break;
            }
            uint8_t fullHash[CC_SHA384_DIGEST_LENGTH];
            CC_SHA384(page, (CC_LONG)pageSize, fullHash);
            memcpy(pageHashOut, fullHash, codeDir->hashSize);
            break;
        }
...
bool csd_code_directory_verify_code_slot(CS_DecodedBlob *codeDirBlob, MachO *macho, int slot) {
    ...
    if (!code_directory_calculate_page_hash(&codeDir, macho, slot, slotHash))
        return false;
...
char *csd_code_directory_copy_team_id(CS_DecodedBlob *codeDirBlob, uint32_t *offsetOut) {
    ...
    if (codeDir.version < 0x20200)
        return NULL;
    if (codeDir.teamOffset == 0)
        return NULL;

    char *teamId = NULL;
    csd_blob_read_string(codeDirBlob, codeDir.teamOffset, &teamId);
    if (offsetOut)
        *offsetOut = codeDir.teamOffset;
    return teamId;
}
...
int csd_code_directory_update_special_slots(CS_DecodedBlob *codeDirBlob,
                                            CS_DecodedBlob *xmlEntitlements,
                                            CS_DecodedBlob *derEntitlements,
                                            CS_DecodedBlob *requirements) {
    CS_CodeDirectory codeDir;
    ...
    if (!hashLen) {
        fprintf(stderr, "Error: unknown hash type (%d)\n", codeDir.hashType);
    }
    ...
    for (int i = 1; i <= codeDir.nSpecialSlots; i++) {
        ...
        if (xmlEntitlements && i == CSSLOT_ENTITLEMENTS) {
            ...
            hashFunc(memory_stream_get_raw_pointer(xmlEntitlements->stream),
                     memory_stream_get_size(xmlEntitlements->stream),
                     newHash);
            csd_blob_write(codeDirBlob, hashOffset, codeDir.hashSize, newHash);
        } else if (derEntitlements && i == CSSLOT_DER_ENTITLEMENTS) {
            hashFunc(memory_stream_get_raw_pointer(derEntitlements->stream),
                     memory_stream_get_size(derEntitlements->stream),
                     newHash);
            csd_blob_write(codeDirBlob, hashOffset, codeDir.hashSize, newHash);
        } else if (requirements && i == CSSLOT_REQUIREMENTS) {
            hashFunc(memory_stream_get_raw_pointer(requirements->stream),
                     memory_stream_get_size(requirements->stream),
                     newHash);
            csd_blob_write(codeDirBlob, hashOffset, codeDir.hashSize, newHash);
        }
    }
...
CS_DecodedBlob *csd_code_directory_init(MachO *macho, int hashType, bool alternate) {
    ...
    int finalLength = sizeof(CS_CodeDirectory) + (newCodeDir.nSpecialSlots * newCodeDir.hashSize)
        + (newCodeDir.nCodeSlots * newCodeDir.hashSize);
