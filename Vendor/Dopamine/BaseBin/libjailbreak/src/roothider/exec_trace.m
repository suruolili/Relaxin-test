...
static void finish_process_trace(trace_data_t *trace_data, bool success) {
    ...
    if (!success) {
        ...
        for (uint32_t i = 0; i < trace_data->saved_exception_types_count; ++i) {
            task_set_exception_ports(trace_data->task,
...
static void *exception_server(void *arg) {
    ...
    while (true) {
        ...
        if (ret != MACH_MSG_SUCCESS) {
            JBLogError("mach_msg error=%x", ret);
            usleep(10 * 1000);
            continue;
        }
        ...
        if (!trace_data) {
            JBLogError("no trace data for pid=%d, %s", pid, proc_get_path(pid, NULL));
            ...
            switch (request->code[1]) {
                case SIGSTOP: {
                    bool data = true;
                    kern_return_t kr = vm_write(request->task.name,
                                                trace_data->traced_flag_addr,
                                                (mach_vm_address_t)&data,
                                                sizeof(data));
                    if (kr != KERN_SUCCESS) {
                        JBLogError("vm_write error: %x, %s", kr, mach_error_string(kr));
                    ...
                    if (ret != 0) {
                        JBLogError("PT_CONTINUE error: %d, %s", errno, strerror(errno));
                        finish_process_trace(trace_data, false);
                        trace_data = NULL;
                        break;
                    }
                ...
                case SIGTRAP: {
                    if (trace_data->cancelled == false) {
                        if (roothide_patch_proc(pid) != 0) {
                            JBLogError("roothide_patch_proc failed for pid=%d, %s", pid, proc_get_path(pid, NULL));
                            finish_process_trace(trace_data, false);
                            trace_data = NULL;
                            break;
                        }
                    ...
                    for (uint32_t i = 0; i < trace_data->saved_exception_types_count; ++i) {
                        kern_return_t kr = task_set_exception_ports(request->task.name,
                                                                    trace_data->saved_masks[i],
                                                                    trace_data->saved_ports[i],
                                                                    trace_data->saved_behaviors[i],
                                                                    trace_data->saved_flavors[i]);
                        if (kr != KERN_SUCCESS) {
                            JBLogError("task_set_exception_ports[%d] error: %x, %s", i, kr, mach_error_string(kr));
                    ...
                    if (ret != 0) {
                        JBLogError("PT_DETACH error: %d, %s", errno, strerror(errno));
                        finish_process_trace(trace_data, false);
                        trace_data = NULL;
                        break;
                    }
            ...
            JBLogError("unexpected exception type: %d from %d,%s", request->exception, pid, proc_get_path(pid, NULL));
        }
...
int execTraceProcess(pid_t pid, uint64_t traced) {
    ...
    kr = task_get_exception_ports(task,
                                  EXC_MASK_SOFTWARE,
                                  trace_data->saved_masks,
                                  &trace_data->saved_exception_types_count,
                                  trace_data->saved_ports,
                                  trace_data->saved_behaviors,
                                  trace_data->saved_flavors);
    if (kr == KERN_SUCCESS) {
