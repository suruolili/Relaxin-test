...
int boomerang_get_physrw(audit_token_t *clientToken,
    ...
    if (!singlePTE && system_info_uses_sptm()) {
        JBLogError("boomerang full-PHYRW handoff rejected pid=%d status=%d", pid, ENOTSUP);
        return ENOTSUP;
    }

    thread_caffeinate_start();
    if (singlePTE) {
        r = physrw_pte_handoff(pid, pageTableVirtualAddress, singlePTEAsidPtr);
    } else {
...
struct jbserver_impl gBoomerangServer = {.maxDomain = 1,
