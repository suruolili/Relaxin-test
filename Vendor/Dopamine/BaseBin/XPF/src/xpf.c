...
XPFSet gBaseSet = {.name = "base",
                   .supported = xpf_supported_always,
                   .metrics = {"kernelConstant.kernel_el",
                               "kernelSymbol.allproc",
                               "kernelSymbol.kalloc_data_external",
                               "kernelSymbol.kfree_data_external",
                               NULL}};

XPFSet gTranslationSet = {.name = "translation",
                          .supported = xpf_supported_always,
                          .metrics = {"kernelSymbol.cpu_ttep",
...
XPFSet gPhysmapSet = {.name = "physmap",
                      .supported = xpf_supported_always,
                      .metrics = {"kernelSymbol.vm_page_array_beginning_addr",
...
XPFSet gStructSet = {.name = "struct",
                     .supported = xpf_supported_always,
                     .metrics = {"kernelStruct.proc.struct_size",
                                 "kernelStruct.task.itk_space",
                                 "kernelStruct.vm_map.pmap",
                                 NULL}};

XPFSet gTrustcache15Set = {.name = "trustcache",
                           .supported = xpf_supported_15down,
                           {"kernelSymbol.pmap_image4_trust_caches", NULL}};

XPFSet gTrustcache16Set = {.name = "trustcache",
                           xpf_supported_16up,
                           .metrics = {"kernelSymbol.ppl_trust_cache_rt",
                                       "kernelSymbol.boot_os_tc_loaded",
                                       "kernelSymbol.boot_app_tc_loaded",
                                       NULL}};

XPFSet gBadRecoverySet = {.name = "badRecovery",
                          .supported = xpf_bad_recovery_supported,
                          .metrics = {"kernelSymbol.hw_lck_ticket_reserve_orig_allow_invalid",
...
XPFSet gSptmFugu14Set = {.name = "sptmFugu14",
                         .supported = xpf_sptm_fugu14_supported,
                         .metrics = {"kernelSymbol.exception_return",
                                     "kernelGadget.exception_return_after_check",
                                     "kernelGadget.str_x8_x9",
                                     "kernelGadget.str_x0_braa_x16_x17",
                                     "kernelGadget.pacia_x16_x17_store_x0",
                                     "kernelGadget.pacda_x16_x17_store_x0",
                                     "kernelGadget.pacibsp_store_lr_return",
                                     "kernelSymbol.ml_sign_thread_state",
                                     "kernelStruct.thread.machine_kstackptr",
                                     "kernelStruct.thread.machine_contextData",
                                     "kernelConstant.TH_KSTACKPTR_DIVERSIFIER",
                                     "kernelSymbol.flush_mmu_tlb_region_asid_async",
                                     "kernelSymbol.mac_label_set",
                                     NULL}};

XPFSet gSandboxSet = {.name = "sandbox",
...
XPFSet gPerfKRWSet = {
    ...
    .metrics = {"kernelSymbol.perfmon_dev_open",
...
XPFSet gDevModeSet = {
    ...
    .metrics = {"kernelSymbol.developer_mode_enabled", NULL},
};
...
XPFSet gTrigonSet = {.name = "trigon", .supported = xpf_trigon_supported, .metrics = {"kernelSymbol.iorvbar", NULL}};

XPFSet gNameCacheSet = {
    .name = "namecache",
    .supported = xpf_supported_always,
    .metrics = {"kernelSymbol.nchashtbl", "kernelSymbol.nchashmask", NULL},
};

XPFSet gAMFIOidsSet = {
    .name = "amfi_oids",
    .supported = xpf_supported_16up,
    .metrics = {"kernelSymbol.launch_env_logging", "kernelSymbol.developer_mode_status", NULL},
};
...
PFSection *xpf_pfsec_init(const char *filesetEntryId, const char *segName, const char *sectName) {
    PFSection *section = pfsec_init_from_macho(gXPF.kernel,
                                               gXPF.kernelIsFileset ? filesetEntryId : NULL,
                                               segName,
                                               sectName);
    if (section) {
...
int xpf_start_with_kernel_path(const char *kernelPath) {
    ...
    if (gXPF.kernelIsFileset) {
        gXPF.kernelAMFITextSection = xpf_pfsec_init("com.apple.driver.AppleMobileFileIntegrity",
                                                    "__TEXT_EXEC",
                                                    "__text");
        gXPF.kernelAMFIStringSection = xpf_pfsec_init("com.apple.driver.AppleMobileFileIntegrity",
                                                      "__TEXT",
                                                      "__cstring");
        ...
        gXPF.kernelSandboxOSLogSection = xpf_pfsec_init("com.apple.security.sandbox", "__TEXT", "__os_log");
                                      ...
                                      if (loadCommand.cmd == LC_UNIXTHREAD) {
                                          ...
                                          while (cmdData < cmdDataEnd) {
                                              ...
                                              if (flavor == ARM_THREAD_STATE64) {
                                                  arm_thread_state64_t *threadState = (arm_thread_state64_t *)(cmdData
                                                                                                               + 8);
#ifdef __arm64e__
    ...
    PFPatternMetric *versionMetric = pfmetric_pattern_init((void *)versionSearchString,
                                                           NULL,
                                                           strlen(versionSearchString),
                                                           sizeof(uint8_t));
    ...
    if (gXPF.kernelInfoPlistSection) {
        ...
        PFPatternMetric *infoPlistMetric = pfmetric_pattern_init((void *)infoPlistString,
                                                                 NULL,
                                                                 strlen(infoPlistString),
                                                                 sizeof(uint8_t));
    ...
    if (!gXPF.kernelVersionString) {
        ...
        sscanf(gXPF.kernelVersionString,
               "Darwin Kernel Version %[^:]: %*[^;]; root:xnu-%[^/]/%s",
               darwinVersion,
               xnuBuild,
               xnuPlatform);
...
xpc_object_t xpf_construct_offset_dictionary(const char *sets[]) {
    ...
    if (xpf_offset_dictionary_add_set(offsetDictionary, &gBaseSet) != 0)
        return NULL;
...
void xpf_stop(void) {
    ...
    if (gXPF.kernelPPLTextSection)
        pfsec_free(gXPF.kernelPPLTextSection);
    if (gXPF.kernelStringSection)
        pfsec_free(gXPF.kernelStringSection);
    if (gXPF.kernelConstSection)
        pfsec_free(gXPF.kernelConstSection);
    if (gXPF.kernelDataConstSection)
        pfsec_free(gXPF.kernelDataConstSection);
    if (gXPF.kernelDataSection)
        pfsec_free(gXPF.kernelDataSection);
    if (gXPF.kernelOSLogSection)
        pfsec_free(gXPF.kernelOSLogSection);
    if (gXPF.kernelAMFITextSection)
        pfsec_free(gXPF.kernelAMFITextSection);
    if (gXPF.kernelAMFIStringSection)
        pfsec_free(gXPF.kernelAMFIStringSection);
    if (gXPF.kernelAMFIDataSection)
        pfsec_free(gXPF.kernelAMFIDataSection);
    if (gXPF.kernelSandboxTextSection)
        pfsec_free(gXPF.kernelSandboxTextSection);
    if (gXPF.kernelSandboxStringSection)
        pfsec_free(gXPF.kernelSandboxStringSection);
    if (gXPF.kernelSandboxOSLogSection)
        pfsec_free(gXPF.kernelSandboxOSLogSection);
    if (gXPF.kernelPrelinkTextSection)
        pfsec_free(gXPF.kernelPrelinkTextSection);
    if (gXPF.kernelPrelinkDataSection)
        pfsec_free(gXPF.kernelPrelinkDataSection);
    if (gXPF.kernelBootdataInit)
        pfsec_free(gXPF.kernelBootdataInit);
    if (gXPF.kernelPLKTextSection)
        pfsec_free(gXPF.kernelPLKTextSection);
    if (gXPF.kernelInfoPlistSection)
    ...
    if (gXPF.kernelVersionString)
        free(gXPF.kernelVersionString);
    if (gXPF.darwinVersion)
        free(gXPF.darwinVersion);
    if (gXPF.xnuBuild)
        free(gXPF.xnuBuild);
    if (gXPF.xnuPlatform)
        free(gXPF.xnuPlatform);
    if (gXPF.osVersion)
        free(gXPF.osVersion);
    if (gXPF.kernelInfoPlist)
        free(gXPF.kernelInfoPlist);
