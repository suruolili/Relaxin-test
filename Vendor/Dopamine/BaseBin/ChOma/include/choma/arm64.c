...
int arm64_gen_b_l(optional_bool optIsBl,
                  optional_uint64_t optOrigin,
                  optional_uint64_t optTarget,
                  uint32_t *bytesOut,
                  uint32_t *maskOut) {
    bool isBl = false;
...
int arm64_gen_b_c_cond(optional_bool optIsBc,
                       optional_uint64_t optOrigin,
                       optional_uint64_t optTarget,
                       arm64_cond optCond,
                       uint32_t *bytesOut,
...
int arm64_gen_adr_p(optional_bool optIsAdrp,
                    optional_uint64_t optOrigin,
                    optional_uint64_t optTarget,
                    arm64_register reg,
                    uint32_t *bytesOut,
                    uint32_t *maskOut) {
    if (ARM64_REG_IS_ANY_VECTOR(reg))
        return -1;
...
int arm64_dec_adr_p(uint32_t inst, uint64_t origin, uint64_t *targetOut, arm64_register *registerOut, bool *isAdrpOut) {
    ...
    if (registerOut)
        *registerOut = ARM64_REG_X(inst & 0x1f);
    if (isAdrpOut)
        *isAdrpOut = isAdrp;
    return 0;
}

int arm64_gen_mov_imm(char type,
                      arm64_register destinationReg,
                      optional_uint64_t optImm,
                      optional_uint64_t optShift,
                      uint32_t *bytesOut,
                      uint32_t *maskOut) {
    if (ARM64_REG_IS_ANY_VECTOR(destinationReg))
        return -1;
...
int arm64_dec_mov_imm(uint32_t inst,
                      arm64_register *destinationRegOut,
                      uint64_t *immOut,
                      uint64_t *shiftOut,
                      char *typeOut) {
...
int arm64_gen_mov_reg(arm64_register destinationReg, arm64_register sourceReg, uint32_t *bytesOut, uint32_t *maskOut) {
    ...
    if (!ARM64_REG_IS_ANY(destinationReg) && !ARM64_REG_IS_ANY(sourceReg)) {
        if (ARM64_REG_IS_W(destinationReg) != ARM64_REG_IS_W(sourceReg))
            return -1;
    }
...
int arm64_dec_mov_reg(uint32_t inst, arm64_register *destinationRegOut, arm64_register *sourceRegOut) {
    ...
    if (destinationRegOut)
        *destinationRegOut = ARM64_REG(is64 ? ARM64_REG_TYPE_X : ARM64_REG_TYPE_W, inst & 0x1f);
    if (sourceRegOut)
        *sourceRegOut = ARM64_REG(is64 ? ARM64_REG_TYPE_X : ARM64_REG_TYPE_W, (inst >> 16) & 0x1f);

    return 0;
}

int arm64_gen_add_imm(arm64_register destinationReg,
    ...
    if (ARM64_REG_IS_ANY_VECTOR(destinationReg))
        return -1;
    if (ARM64_REG_IS_ANY_VECTOR(sourceReg))
        return -1;
...
int arm64_dec_add_imm(uint32_t inst,
                      arm64_register *destinationRegOut,
                      arm64_register *sourceRegOut,
                      uint16_t *immOut) {
    if ((inst & 0x7f800000) != 0x11000000)
        return -1;
...
int arm64_gen_sub_imm(arm64_register destinationReg,
                      arm64_register sourceReg,
                      optional_uint64_t optImm,
                      optional_bool optS,
                      uint32_t *bytesOut,
...
int arm64_dec_sub_imm(uint32_t inst,
                      arm64_register *destinationRegOut,
                      arm64_register *sourceRegOut,
                      uint16_t *immOut,
                      bool *sOut) {
...
uint8_t arm64_str_ldr_type_width(arm64_register_type registerType, char ldrStrType) {
    switch (ldrStrType) {
        ...
        case 'h':
            return 2;
        default:
            return arm64_reg_type_get_width(registerType);
    }
}

static int _arm64_gen_str_ldr_imm(uint32_t inst,
                                  uint32_t mask,
                                  char type,
                                  arm64_ldr_str_type instType,
                                  arm64_register sourceDestinationReg,
                                  arm64_register addrReg,
                                  optional_uint64_t optImm,
                                  uint32_t *bytesOut,
                                  uint32_t *maskOut) {
    if (ARM64_REG_IS_ANY_VECTOR(addrReg))
        return -1;
    ...
    if (OPT_UINT64_IS_SET(optImm)) {
        uint64_t imm = OPT_UINT64_GET_VAL(optImm)
            / arm64_str_ldr_type_width(ARM64_REG_GET_TYPE(sourceDestinationReg), type);
        if (imm & ~0xfff)
            return -1;
        inst |= (imm << 10);
        mask |= (0xfff << 10);
...
static int _arm64_dec_str_ldr_imm(uint32_t inst,
                                  arm64_register *sourceDestinationRegOut,
                                  arm64_register *addrRegOut,
                                  uint64_t *immOut,
                                  char *typeOut,
                                  arm64_ldr_str_type *instTypeOut) {
...
int arm64_gen_ldr_imm(char type,
                      arm64_ldr_str_type instType,
                      arm64_register destinationReg,
                      arm64_register addrReg,
                      optional_uint64_t optImm,
                      uint32_t *bytesOut,
                      uint32_t *maskOut) {
    return _arm64_gen_str_ldr_imm(0x38400000,
                                  0x3a400000,
                                  type,
                                  instType,
                                  destinationReg,
...
int arm64_dec_ldr_imm(uint32_t inst,
                      arm64_register *destinationRegOut,
                      arm64_register *addrRegOut,
                      uint64_t *immOut,
                      char *typeOut,
                      arm64_ldr_str_type *instTypeOut) {
    if ((inst & 0x3a400000) != 0x38400000)
        return -1;
    return _arm64_dec_str_ldr_imm(inst, destinationRegOut, addrRegOut, immOut, typeOut, instTypeOut);
}

int arm64_gen_ldrs_imm(char type,
                       arm64_ldr_str_type instType,
                       arm64_register destinationReg,
                       arm64_register addrReg,
                       optional_uint64_t optImm,
                       uint32_t *bytesOut,
                       uint32_t *maskOut) {
    return _arm64_gen_str_ldr_imm(0x38800000,
                                  0x3ac00000,
                                  type,
                                  instType,
                                  destinationReg,
...
int arm64_dec_ldrs_imm(uint32_t inst,
                       arm64_register *destinationRegOut,
                       arm64_register *addrRegOut,
                       uint64_t *immOut,
                       char *typeOut,
                       arm64_ldr_str_type *instTypeOut) {
    if ((inst & 0x3ac00000) != 0x38800000)
        return -1;
    return _arm64_dec_str_ldr_imm(inst, destinationRegOut, addrRegOut, immOut, typeOut, instTypeOut);
}

int arm64_gen_str_imm(char type,
                      arm64_ldr_str_type instType,
                      arm64_register sourceReg,
                      arm64_register addrReg,
                      optional_uint64_t optImm,
                      uint32_t *bytesOut,
                      uint32_t *maskOut) {
    return _arm64_gen_str_ldr_imm(0x38000000,
                                  0x3a400000,
                                  type,
                                  instType,
                                  sourceReg,
...
int arm64_dec_str_imm(uint32_t inst,
                      arm64_register *sourceRegOut,
                      arm64_register *addrRegOut,
                      uint64_t *immOut,
                      char *typeOut,
                      arm64_ldr_str_type *instTypeOut) {
    if ((inst & 0x3a400000) != 0x38000000)
        return -1;
    return _arm64_dec_str_ldr_imm(inst, sourceRegOut, addrRegOut, immOut, typeOut, instTypeOut);
}

int arm64_gen_ldr_lit(arm64_register destinationReg,
                      optional_uint64_t optOrigin,
                      optional_uint64_t optTarget,
                      uint32_t *bytesOut,
                      uint32_t *maskOut) {
...
int arm64_gen_ldp(arm64_ldp_stp_type instType,
                  arm64_register destinationReg1,
                  arm64_register destinationReg2,
                  arm64_register addrReg,
                  optional_uint64_t optImm,
                  uint32_t *bytesOut,
                  uint32_t *maskOut) {
...
int arm64_gen_cb_n_z(optional_bool isCbnz,
                     arm64_register reg,
                     optional_uint64_t optTarget,
                     uint32_t *bytesOut,
                     uint32_t *maskOut) {
...
int arm64_gen_tb_n_z(optional_bool isTbnz,
                     arm64_register reg,
                     optional_uint64_t optTarget,
                     optional_uint64_t optBit,
                     uint32_t *bytesOut,
                     uint32_t *maskOut) {
    if (ARM64_REG_IS_ANY_VECTOR(reg))
        return -1;
...
int arm64_dec_tb_n_z(uint32_t inst,
                     uint64_t origin,
                     bool *isTbnzOut,
                     arm64_register *regOut,
                     uint64_t *targetOut,
                     uint64_t *bitOut) {
